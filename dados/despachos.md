...
### prioridade ALTA — a robometria está publicando para ninguém: o cache do hospedeiro segura a página e a nuvem não o alcança

14/09/2026 — RAPHAEL — **Isto bloqueia a ilha inteira**, e é a única coisa desta execução que não deu para resolver do lado da máquina. Não é código: é uma purga de cache, ou a chave para fazê-la.

**O que está acontecendo, medido três vezes e não suposto.** A robometria publicou hoje as revisões 37 a 41. O Sync aplicou ("10 aplicado(s)"), o `/status` responde a revisão do manifest, as nove URLs dão 200 e os títulos batem — **e o endereço canônico continua servindo a página anterior.** Lado a lado, na mesma URL, às 16h01Z:

| | endereço canônico | mesmo endereço com `?v=<agora>` |
|---|---|---|
| bytes | 137.943 | 138.832 |
| saídas de compra pela busca | **0** | 4 |
| a frase que a seção 7 proibiu hoje | **4 cartões** | nenhum |
| a tabela diz | "73 **pares**" (o número errado) | "73 **linhas**" (o consertado) |

Ou seja: **o leitor e o Google estão vendo a página de antes do bloco de hoje.** O trabalho está no ar do lado do WordPress e para na última camada.

**O que a Fundação já tentou, e por que parou.** A assinatura no HTML é `<!--Generated by Endurance Page Cache-->`. Foram ao ar, medidos um a um: (1) as duas ações de purga do plugin, disparadas quando a ilha grava qualquer dado — **não mudou nada**; (2) esvaziar a pasta `wp-content/endurance-page-cache/`, que é o que o `purge_all()` do próprio plugin faz — **também não mudou nada, e foi RETIRADA**, porque código que apaga arquivo é o mais arriscado da ilha e não dá para guardar o risco sem o benefício. Depois dela o canônico passou a responder `cache-control: no-store`, **sem** `last-modified` — cabeçalho de página não cacheada — e ainda assim com o corpo antigo. **Isso diz que a camada está à frente do Apache**, e nenhuma linha de PHP a alcança.

**O pedido, e é uma coisa de cada vez:**

1. **Purgar o cache de `robometria.com.br` no painel do hospedeiro** (a purga total, não a de uma URL). É o que destrava o bloco de hoje.
2. Se houver, no painel, **uma opção de purgar por API ou por webhook**, mandar a URL e a chave — elas entram como variável de ambiente da rotina, como o `GOOGLE_SA_B64`, **nunca** em arquivo do repositório. Com isso o Sync passa a purgar sozinho e este despacho não volta.
3. Se não houver nenhuma das duas, dizer qual é a camada (o painel costuma nomeá-la), para a Fundação decidir com o nome certo na mão em vez de tentar às cegas.

**Pronto quando:** `curl -s https://robometria.com.br/qual-peca-serve-no-meu-robo-aspirador/ | grep -c rbm-comprar-cru` devolver o mesmo número que o mesmo comando com `?v=<qualquer coisa>` no fim da URL. Quem confere é `python3 ferramentas/conferir-no-ar.py`, seção 10, que já existe e hoje reprova.

**O que NÃO se faz enquanto isso, e vale para toda ilha:** dar bloco por entregue porque o `/status` bateu. A seção 4 do `ARQUIPELAGO.md` foi ampliada hoje com esta cicatriz — Sync aplicado e revisão confirmada **não são entrega** se o canônico serve a página velha.

**E a clubedomosaico serve a mesma assinatura de cache**, então o mesmo pode estar acontecendo com ela — está no despacho seguinte, que é da Fundação.
...
**O que aconteceu, medido:** a revisão 37 da robometria foi aplicada (log do Sync "10 aplicado(s)"), o `/status` respondeu 37 igual ao manifest, as nove URLs deram 200, os títulos bateram — e o **endereço canônico** continuou servindo, por horas, a cópia anterior: sem um botão de compra e com a frase que a seção 7 acabava de proibir em quatro cartões. A mesma URL com chave de quebra de cache mostrava a página nova inteira. A assinatura está no HTML: `<!--Generated by Endurance Page Cache-->`, o cache em arquivo do hospedeiro, que purga quando um POST é salvo no wp-admin e **nunca vê o Sync**, que grava options e atualiza snippets. A regra nova está na seção 4 do `ARQUIPELAGO.md`.
