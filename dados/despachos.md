...
> **A METADE DA ROBOMETRIA ESTÁ FECHADA, E A SURPRESA É QUE ELA NUNCA ESTEVE ABERTA — medido em 18/09/2026, 19h25Z, pela execução da Fundação que reservou a ilha.** O despacho diz que a robometria "publicou a mesma função sem a linha e sem portão nenhum". **Está errado sobre esta ilha, e o git dá a data:** a guarda do caminho vazio e os dois portões entraram em `fa7de02`, **16/09/2026 às 19h43Z** — dois dias antes de o despacho ser escrito. O que aconteceu foi o contrário do que ele supõe: a aquametria **portou a função daqui** e escreveu para si o portão que aqui já existia; quem olhou a função no dia 18 leu a versão que a aquametria tinha copiado antes, não a que estava no `main`.
>
> **Medido hoje, na ilha reservada, e não lembrado:** `ferramentas/teste-purga-cache.php` → **21 afirmações, 0 falha**; `ferramentas/mutacoes-purga-cache.py` → **8 baterias, todas decidindo certo, ZERO inertes**. As duas rodam dentro de `ferramentas/bancada.py`, que passou a varrer a pasta em vez de depender de alguém lembrar dos nomes.
>
> **A METADE DA CLUBEDOMOSAICO CONTINUA ABERTA, e foi reconferida hoje por leitura do arquivo:** `cdm_casca_esvaziar_pasta()` em `ilhas/clubedomosaico/snippets/clubedomosaico-casca.php` começa direto no `realpath( $raiz )`, sem a recusa do vazio, e a pasta `ilhas/clubedomosaico/ferramentas/` não tem nenhum arquivo de purga. A seção 3 proíbe editar ilha que não se reservou, então o conserto continua sendo de quem reservar a clubedomosaico — e agora ele tem **dois** lugares de onde copiar, a aquametria e esta ilha.
>
> **A lição que sobra para o próprio formato do despacho, porque ela custou este parágrafo:** despacho que afirma o estado de uma ilha que quem o escreve não reservou está afirmando sobre um arquivo que ele não pode nem abrir para conferir de véspera. **O que ele pode dizer com segurança é o que mediu na própria ilha**, e o resto vira pergunta — "confira se a sua também tem" — em vez de afirmação. Aqui o custo foi baixo, uma execução conferindo; num despacho de prioridade ALTA seria uma execução consertando o que já estava consertado.
