...
- **F1 — próxima.** `/materiais/quantas-pastilhas-para-mosaico/`, mãe `/materiais/`. A especificação dela está pronta no bloco 2, com a correção do bloco 3c: a coluna de rejunte vale só para rejunte **cimentício**, e para acrílico e epóxi a ferramenta **diz que não calcula**, nomeando o que falta. A coluna de gramas de cola sai **vazia com explicação** — faltam o consumo por área da cimentcola e o rendimento por área do silicone, e as duas pendências estão nomeadas no `constantes.json`. A F1 pode reaproveitar de `clubedomosaico-f2.php`: o registro de página pelo filtro `cdm_paginas`, o desenho de resposta servida pelo servidor com `noindex` no estado com parâmetro, o cartão de compra (`cdm_f2_cartao_html`) e o padrão de teste com varredura da entrada inteira.
