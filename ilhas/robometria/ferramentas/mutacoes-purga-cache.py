...
CASCA = os.path.join("snippets", "robometria-casca.php")
...
def sem_guarda_do_vazio(src):
    ...
    esta rodando. Na aquametria, onde o portao nasceu, foi assim que ele apagou
    os 117 arquivos da pasta da ilha na primeira vez em que rodou."""
...
def gatilho_sem_prefixo(src):
    ...
    alvo = "\tif ( 0 === strpos( (string) $opcao, 'robometria_' ) ) {"
    assert alvo in src
...
def purga_mais_de_uma_vez(src):
    ...
    alvo = ("function robometria_casca_purgar_cache() {\n"
            "\tstatic $ja = false;\n\tif ( $ja ) {\n\t\treturn;\n\t}\n\t$ja = true;\n")
    assert alvo in src
    return src.replace(alvo, "function robometria_casca_purgar_cache() {\n", 1)
...
def raiz_sem_pasta(src):
    ...
    alvo = ("\t\t\trobometria_casca_esvaziar_pasta( $caminho, $real_raiz );\n"
            "\t\t\t@rmdir( $caminho );\n")
    assert alvo in src
    return src.replace(alvo, "\t\t\trobometria_casca_esvaziar_pasta( $caminho, $real_raiz );\n", 1)
...
def rodar(casca_fonte):
    ...
    tmp = tempfile.mkdtemp(prefix="rbm-mut-")
    try:
