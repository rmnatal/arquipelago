...
## 2026-09-16, 16h18Z–17h15Z — Dois despachos do Raphael, inteiros: o lugar vazio da foto, e a Open API da Shopee

**A ESCOLHA DA ILHA: PELO FOCO DA 1.2, SEM CORRIDA.** `foco.md` nomeia robometria desde 16/09, então não houve rotação a aplicar. `executando_desde: null`, que pela 1.1 já significa que nenhum bloco da Fundação está vivo; nenhum branch `claude/*` fora do main e nenhum PR aberto. Reserva aceita de primeira às 16h18Z e **renovada às 16h32Z**, pela regra 1.1 (bloco longo reescreve o relógio). Rede pela 20.2 antes de trabalhar: home e `/status` em **200 nas três passadas**.

**DESPACHO 1 (15h17Z) — O ESPAÇO VAZIO DA FOTO ESTAVA DESENHADO COMO UM SPINNER.** `border-radius:50%` com um lado transparente é a forma universal da roda de carregamento. Quem chegava não lia "esta peça não tem foto"; lia "a foto está carregando" e, como ela nunca carrega, lia "este site está quebrado" — numa ilha cujo argumento inteiro é procedência. Uma regra de CSS na casca conserta as quatro ferramentas. **E a edição 2 era o defeito que só apareceria depois:** R2 e A2 emitiam o painel SEMPRE, sem perguntar nada ao item; não aparecia porque nenhum registro tinha foto. **Verdade por coincidência do banco** — e o despacho seguinte, no mesmo dia, era justamente o que traria a primeira foto. Casca 1.7.1, manifest 53, `conferir-no-ar` com **250 afirmações e 0 falha**.

**DESPACHO 2 (16h01Z + adendo 16h07Z) — A OPEN API DA SHOPEE.** Saiu inteiro, os quatro itens. `ferramentas/shopee-api.py` na raiz (como `ga4.py`, porque a 25.6 nasce valendo para todas as ilhas); esquema na **versão 9**; coleta de 73 registros; vitrine com foto. Esquema 9, casca 1.8.0, R1 1.10.0, R2 1.8.0, A1 1.4.0, A2 1.4.0, manifest **54**, `conferir-no-ar` com **251 afirmações e 0 falha**.

**A COLETA, MEDIDA E NÃO ESTIMADA:** 27 de 73 publicáveis casaram — 12 peças, 15 modelos. Degraus da escada de palavra-chave: **12 no 1** (o código sozinho), 3 no 2, 8 no 3, 4 no 4. **Os 46 que não casaram ficaram sem foto e sem ficha, com o motivo escrito** — a escada inteira foi percorrida e nenhum resultado traz o código do registro com o tipo certo. Isso é medição honesta, não falha.

**E A CONFERÊNCIA COM OS OLHOS DA 25.3 PAGOU POR SI CINCO VEZES.** Cada uma é um casamento que a regra do despacho — *"o título contém o código da peça ou o nome do modelo"* — aprovaria: (1) *"Tampa Do Filtro"* casando com um filtro e *"Tampa Da Bateria"* com uma bateria; (2) *"Kit Filtro Pinos e Lâmina"* casando com um filtro, e quem comprasse levaria pinos e lâmina; (3) `S10`, `E10`, `W100` e `S40` sozinhos casando com mangueira de ar-condicionado, máscara capilar, whey protein e fechadura de Volvo — e *"Carcaça Inferior"*, *"Placa de Potência"* e *"Controle Remoto"* casando como se fossem o ERB10, o ERB11 e o ERB20; (4) `S40` casando com *"S40 PRO"* e depois com *"S40 C"*, e `S10` com *"S10+"*; (5) dois registros do banco no mesmo anúncio. **As cinco viraram a seção 25.7 do contrato.**

**LARGURA E ALTURA SÃO MEDIDAS.** A API não devolve dimensão, mas `cf.shopee.com.br` responde 200 da nuvem: `coletar-shopee.py --medir` lê os primeiros bytes e parseia o cabeçalho JPEG/PNG/WebP. **27 de 27 medidas.** Sem as duas, a foto não entra — salto de layout é pior que sem foto.

**SETE RÉGUAS AMARRADAS A UM DEGRAU CAÍRAM NO MESMO DIA, e essa é a lição maior desta execução.** De manhã quatro (`teste-r1`, `teste-a1`, `teste-r2`, `teste-a2`) reprovaram a chegada dos links encurtados; corrigidas para cobrar `rbm-comprar-busca`, **as mesmas caíram de novo à tarde** quando a ficha de produto chegou e os cartões subiram outro degrau. Com elas caíram a seção 11 do `conferir-no-ar.py` e o `conferir-reservatorio-no-ar.py` — este porque o **rótulo do botão** muda com o degrau ("Ver ofertas na Shopee" → "Ver na Shopee"). Trocar a classe por outra só adia a próxima queda. Todas passaram a contar **cartão contra bloco de compra**, com o degrau impresso ao lado como informação. Subiu para a seção 8 do contrato.

**E DUAS TRAVAS ESTAVAM DOENTES DE OUTRAS DUAS MANEIRAS.** `itens_com_ficha`, no cabeçalho dos dois bancos e na bateria da escada, **contava o link de afiliado** e se chamava ficha — as duas cópias erradas juntas, dando zero, então ninguém podia ver; virou duas chaves. E `mutacoes-escada` tinha uma mutação **inerte**: ela apagava um `motivo_sem_url_busca` que já estava vazio desde que os modelos ganharam piso. As duas são a mesma família do achado 2 da Sentinela de hoje.

**E UMA TERCEIRA, NO AVESSO:** `conferir-kits-no-ar.py` esperava que Xiaomi E10 × escova lateral **recusasse** — verdade quando foi escrita, falsa desde 14/09, quando a `B106GL-BX` foi alargada. Ele ficou **vermelho sobre uma página certa** e ninguém viu porque ninguém o rodou desde a leva. Também subiu para a seção 8.

**BANCADA, 0 falha:** casca 219, r1 224, r2 107, a1 68, a2 74, árvore 219, voz 155, acentuação 17, purga-cache 21, escada-compra 721, validar-banco e conferir-canal-na-resposta aprovados, `php -l` limpo nos seis snippets. **MUTAÇÕES: 23 baterias, todas no resultado esperado, ZERO inertes** — incluindo a `mutacoes-lugar-vazio-da-foto` nova, reescrita à tarde porque o banco passou a ter os dois mundos de verdade e plantar a foto virou mutação que não morde. **NO AR:** Sync em dois disparos nas duas revisões, `/status` na revisão do manifest, `conferir-no-ar` 251, `conferir-kits-no-ar` 165, `conferir-reservatorio-no-ar` 50, `conferir-atribuicao-no-ar` 42 — todos 0 falha.

**CREDENCIAL:** o `AppID` e a `Senha` vieram do documento privado `arquipelago-credenciais` do Drive, passaram ao processo só como variável de ambiente e **não estão em arquivo, log, relatório nem mensagem de commit** — conferido com `grep` na árvore inteira antes de cada push.

**UMA COISA SAIU TORTA E FICA REGISTRADA:** o commit `82c2e54` perdeu os trechos entre crases da mensagem, porque ela foi passada pelo shell com `-m` e o bash expandiu as crases. O conteúdo está certo, a mensagem está furada, e não há conserto sem reescrever histórico já empurrado — o que esta fábrica proíbe. **Mensagem de commit longa passa a ir por arquivo (`-F`), nunca por `-m`.**
