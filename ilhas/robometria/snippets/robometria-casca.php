 ...
 * duas requisições, dois estados do cache, e um "no-store" que parecia dizer que
 * a camada estava fora do alcance do PHP. Medidos juntos, HEAD e GET concordam.
 * E o relógio desfaz a outra metade: a entrada presa era de 14h40:01 com max-age
 * de 7200 s, então só venceria às 16h40 — e foi recriada às 16h05:59, durante o
 * Sync da revisão 41, com esta função sendo o código que rodava. Não é prova, é
 * a única causa identificada com o vencimento por tempo descartado.
 ...
 * cartões. A assinatura estava no HTML: Endurance Page Cache, o cache em arquivo
 * do hospedeiro, que purga quando um POST é salvo no wp-admin e nunca vê o Sync,
 * que grava options e atualiza snippets. A purga passou a ser disparada pela
 * própria gravação das options da ilha, e mora aqui e não no Sync porque o Sync
 * se pula a si mesmo por desenho. Quem mede é a seção 10 do conferir-no-ar.py,
 * que põe o endereço canônico contra o mesmo endereço com quebra de cache.
...
add_shortcode( 'robometria_afiliados', function () {
	...
	/* A FRASE ANTIGA NÃO É CITADA AQUI, e isso é decisão, não esquecimento: a
	   régua que confere no ar proíbe aquelas palavras no corpo de qualquer
	   página, e a alternativa — marcar a citação no markup para o portão a
	   perdoar — é mecanismo demais para uma frase que o leitor não precisa ler.
	   Quem quiser a história tem o REGISTRO.md e a descrição do manifest. */
	$html .= '<p>Esconder o botão enquanto o link de afiliado não chega seria pior ainda, e foi o que esta página fez até 14 de setembro de 2026: o lugar do botão ficava reservado com um aviso de que o link viria depois. A página ficava com um único endereço clicável — o da declaração do fabricante — e mandava você comprar na loja da própria marca, onde não dá para comparar preço com ninguém.</p>';
	$html .= '</div>';
...
/**
 * O DESEMBARQUE PARAVA NO CACHE DO HOST, E NINGUÉM VIA — 14/09/2026.
 *
 * Achado fechando o item 1 do despacho da Sentinela, e ele é maior do que o item.
 * A ronda mediu que `curl` sem cabeçalho recebia uma cópia de 11/09; medindo a
 * causa, a página cacheada tem assinatura: `<!--Generated by Endurance Page
 * Cache-->`, o cache em arquivo do próprio hospedeiro. Ele é purgado quando um
 * POST é salvo no wp-admin — e o Sync desta ilha não salva post nenhum na maior
 * parte das revisões: ele grava OPTIONS e atualiza SNIPPETS. Resultado medido
 * nesta execução: a revisão 37 aplicou ("10 aplicado(s)"), o `/status` respondeu
 * 37, e a página de compatibilidade continuou servindo, no endereço canônico, a
 * cópia das 14h40 — sem um botão de compra, com a frase proibida pela seção 7 em
 * quatro cartões. Com quebra de cache na URL, a página nova aparecia inteira.
 *
 * ISSO É A SEÇÃO 4 DO CONTRATO UMA CAMADA ABAIXO. Lá, "o repositório estava na
 * revisão 17 e o site na 11" porque ninguém acionava o Sync. Aqui o Sync é
 * acionado, o log diz aplicado, o `/status` confirma a revisão — e o leitor
 * continua na página de antes. "Aplicado com sucesso no log do Sync não é
 * evidência de nada" ganhou um segundo significado.
 *
 ...
 * mesmo. A casca é publicada a cada revisão, e ela já está carregada durante a
 ...
 * defensivo é a verificação: quem confere é ferramentas/conferir-no-ar.py, que
 * compara o endereço canônico com o mesmo endereço com quebra de cache e reprova
 * quando os dois discordam. Purga que ninguém mede é purga que envelhece calada.
 */
if ( ! function_exists( 'robometria_casca_purgar_cache' ) ) {
function robometria_casca_purgar_cache() {
	static $ja = false;
	if ( $ja ) {
		return;
	}
	...
	/* Endurance Page Cache — o que este site usa hoje, medido pela assinatura
	...
	/* A PURGA QUE NÃO DEPENDE DO NOME DO GANCHO — e ela foi RETIRADA e RESTAURADA
	   na mesma execução, em 14/09/2026, o que vale mais registrado do que
	   escondido.

	   Ela esvazia `wp-content/endurance-page-cache/`, que é o que o `purge_all()`
	   do próprio plugin faz. Foi ao ar na revisão 40 e, medida logo depois,
	   pareceu não fazer efeito: o endereço canônico continuou servindo a página
	   anterior. Com isso ela foi retirada na 41 — código que apaga arquivo é o
	   mais arriscado desta ilha, e risco sem benefício não se guarda.

	   A LEITURA ESTAVA ERRADA, E O ERRO TEM NOME. A medição que a condenou
	   comparava cabeçalhos obtidos com `curl -I` (HEAD) com um corpo obtido em
	   OUTRA requisição (GET), segundos depois. Naquela janela as duas pegaram
	   estados diferentes do cache, e o `no-store` sem `last-modified` que parecia
	   dizer "isto não é arquivo local" era o retrato de um arquivo que estava
	   justamente sendo apagado. Medidos juntos depois, HEAD e GET concordam.

	   O QUE A LINHA DO TEMPO DIZ, e é o que sustenta a restauração: a entrada de
	   cache presa era de 14h40:01 e o `max-age` é de 7200 s, ou seja ela só
	   venceria sozinha às 16h40. **Ela foi recriada às 16h05:59**, durante o Sync
	   da revisão 41 — e o código rodando naquele instante era exatamente esta
	   função. Não é prova: é a única causa identificada, com o vencimento por
	   tempo descartado pelo relógio.

	   O TESTE QUE DECIDE, para quem vier depois: publicar uma revisão que mude
	   texto servido e conferir se o endereço canônico muda em minutos. Se mudar
	   com esta função presente e parar de mudar sem ela, a atribuição vira
	   medição. Quem mede é a seção 10 do `conferir-no-ar.py`.

	   AS TRÊS GUARDAS, e nenhuma é decoração: o caminho é montado a partir de
	   WP_CONTENT_DIR e nunca de entrada de requisição; o caminho real é conferido
	   contra a raiz a cada nível da recursão; e link simbólico não é seguido,
	   para não sair da pasta por um atalho que alguém tenha deixado lá. */
 ...
 *
 * O Sync escreve `robometria_dados_*` e `robometria_sync_estado` a cada
 * aplicação, então uma revisão que muda QUALQUER coisa passa por aqui. O prefixo
 * é conferido para a purga não disparar em gravação de option de terceiro — e o
 * `static` da função acima faz a purga acontecer UMA vez por requisição, mesmo
 * quando a revisão grava dez options.
 */
if ( ! function_exists( 'robometria_casca_purgar_ao_gravar' ) ) {
function robometria_casca_purgar_ao_gravar( $opcao ) {
	if ( 0 === strpos( (string) $opcao, 'robometria_' ) ) {
		robometria_casca_purgar_cache();
	}
}
}
add_action( 'updated_option', 'robometria_casca_purgar_ao_gravar', 10, 1 );
add_action( 'added_option', 'robometria_casca_purgar_ao_gravar', 10, 1 );
