 ...
 *
 * Puxa o manifest.json da ilha Robometria no repositório público rmnatal/arquipelago
 ...
 *   - dados     → grava JSON na option robometria_dados_{id} (autoload off)
 ...
 *
 * Gatilhos: WP-Cron a cada 30 min; GET /?robometria_sync=TOKEN[&forcar=1];
 *           REST POST /wp-json/robometria/v1/sync (manage_options).
 * Leitura:  REST GET /wp-json/robometria/v1/status (público, só o log) e
 *           REST GET /wp-json/robometria/v1/token (manage_options).
 *
...
if ( ! defined( 'ROBOMETRIA_SYNC_VERSAO' ) ) {
	define( 'ROBOMETRIA_SYNC_VERSAO', '1.1.5' );
	define( 'ROBOMETRIA_SYNC_BASE', 'https://raw.githubusercontent.com/rmnatal/arquipelago/main/ilhas/robometria/' );
	define( 'ROBOMETRIA_SYNC_NOME_PROPRIO', 'Robometria Sync' );
}

/* ---------- utilidades ---------- */

if ( ! function_exists( 'robometria_sync_token' ) ) {
function robometria_sync_token() {
	$t = get_option( 'robometria_sync_token' );
	if ( ! $t ) {
		$t = wp_generate_password( 32, false, false );
		add_option( 'robometria_sync_token', $t, '', 'no' );
	}
	return $t;
}
}

if ( ! function_exists( 'robometria_sync_estado' ) ) {
function robometria_sync_estado() {
	$e = get_option( 'robometria_sync_estado' );
	if ( ! is_array( $e ) ) {
...
if ( ! function_exists( 'robometria_sync_log' ) ) {
function robometria_sync_log( &$estado, $msg ) {
...
if ( ! function_exists( 'robometria_sync_baixar' ) ) {
function robometria_sync_baixar( $rel ) {
	$url = ROBOMETRIA_SYNC_BASE . ltrim( $rel, '/' ) . '?v=' . time();
...
if ( ! function_exists( 'robometria_sync_sha_ok' ) ) {
function robometria_sync_sha_ok( $corpo, $esperado ) {
	if ( empty( $esperado ) ) {
...
if ( ! function_exists( 'robometria_sync_md' ) ) {
function robometria_sync_md( $md ) {
...
if ( ! function_exists( 'robometria_sync_aplicar_snippet' ) ) {
function robometria_sync_aplicar_snippet( $item, &$estado ) {
	...
	if ( '' === $nome || $nome === ROBOMETRIA_SYNC_NOME_PROPRIO ) {
		return 'pulado (nome vazio ou é o próprio Sync)';
	}
	$corpo = robometria_sync_baixar( $item['arquivo'] );
	if ( is_wp_error( $corpo ) ) {
		return 'erro download: ' . $corpo->get_error_message();
	}
	if ( ! robometria_sync_sha_ok( $corpo, isset( $item['sha256'] ) ? $item['sha256'] : '' ) ) {
...
if ( ! function_exists( 'robometria_sync_aplicar_conteudo' ) ) {
function robometria_sync_aplicar_conteudo( $item, &$estado ) {
	$corpo = robometria_sync_baixar( $item['arquivo'] );
	if ( is_wp_error( $corpo ) ) {
		return 'erro download: ' . $corpo->get_error_message();
	}
	...
	$html     = ( '.html' === substr( $item['arquivo'], -5 ) ) ? $corpo : robometria_sync_md( $corpo );
	...
	update_post_meta( $pid, '_robometria_id', $item['id'] );
	update_post_meta( $pid, '_robometria_cluster', isset( $item['cluster'] ) ? $item['cluster'] : '' );
	update_post_meta( $pid, '_robometria_verificado_em', isset( $item['verificado_em'] ) ? $item['verificado_em'] : '' );
	update_post_meta( $pid, '_robometria_fontes', isset( $item['fontes'] ) ? wp_json_encode( $item['fontes'] ) : '' );
...
if ( ! function_exists( 'robometria_sync_aplicar_dados' ) ) {
function robometria_sync_aplicar_dados( $item, &$estado ) {
	$corpo = robometria_sync_baixar( $item['arquivo'] );
	if ( is_wp_error( $corpo ) ) {
		return 'erro download: ' . $corpo->get_error_message();
	}
	...
	$chave = 'robometria_dados_' . sanitize_key( $item['id'] );
...
if ( ! function_exists( 'robometria_sync_executar' ) ) {
function robometria_sync_executar( $forcar = false ) {
	$estado = robometria_sync_estado();
	$bruto  = robometria_sync_baixar( 'manifest.json' );
	if ( is_wp_error( $bruto ) ) {
		robometria_sync_log( $estado, 'FALHA manifest: ' . $bruto->get_error_message() );
		update_option( 'robometria_sync_estado', $estado, 'no' );
		return $estado;
	}
	$m = json_decode( $bruto, true );
	if ( ! is_array( $m ) || ! isset( $m['revisao'] ) ) {
		robometria_sync_log( $estado, 'FALHA manifest: JSON inválido' );
		update_option( 'robometria_sync_estado', $estado, 'no' );
		return $estado;
	}
	...
	foreach ( array( 'snippets', 'conteudo', 'dados' ) as $grupo ) {
		...
		foreach ( $m[ $grupo ] as $item ) {
			...
			try {
				if ( 'snippets' === $grupo ) {
					$res = robometria_sync_aplicar_snippet( $item, $estado );
				} elseif ( 'conteudo' === $grupo ) {
					$res = robometria_sync_aplicar_conteudo( $item, $estado );
				} else {
					$res = robometria_sync_aplicar_dados( $item, $estado );
				}
			} catch ( \Throwable $e ) {
				$res = 'ERRO ' . get_class( $e ) . ': ' . $e->getMessage() . ' (' . basename( $e->getFile() ) . ':' . $e->getLine() . ')';
			}
			robometria_sync_log( $estado, $grupo . '/' . $item['id'] . ': ' . $res );
			if ( 0 === strpos( $res, 'ok' ) ) {
				$aplicados++;
			}
		}
	...
	$estado['ultimo']  = current_time( 'mysql' );
	robometria_sync_log( $estado, 'revisão ' . $m['revisao'] . ': ' . $aplicados . ' aplicado(s), ' . $pulados . ' aguardando desembarque' . ( $forcar ? ' [forçado]' : '' ) );
	update_option( 'robometria_sync_estado', $estado, 'no' );
...
add_filter( 'cron_schedules', function ( $s ) {
	$s['robometria_30min'] = array( 'interval' => 1800, 'display' => 'Robometria: a cada 30 min' );
	return $s;
} );

add_action( 'init', function () {
	if ( ! wp_next_scheduled( 'robometria_sync_evento' ) ) {
		wp_schedule_event( time() + 120, 'robometria_30min', 'robometria_sync_evento' );
	}
	$token = filter_input( INPUT_GET, 'robometria_sync', FILTER_DEFAULT );
	if ( is_string( $token ) && '' !== $token ) {
		if ( ! hash_equals( robometria_sync_token(), $token ) ) {
			status_header( 403 );
			exit( 'token inválido' );
		}
		$forcar = '1' === (string) filter_input( INPUT_GET, 'forcar', FILTER_DEFAULT );
		$e      = robometria_sync_executar( $forcar );
		nocache_headers();
		header( 'Content-Type: application/json; charset=utf-8' );
		echo wp_json_encode( array( 'versao_sync' => ROBOMETRIA_SYNC_VERSAO, 'revisao' => $e['revisao'], 'ultimo' => $e['ultimo'], 'log' => array_slice( $e['log'], 0, 10 ) ), JSON_UNESCAPED_UNICODE );
		exit;
	}
} );

add_action( 'robometria_sync_evento', function () {
	robometria_sync_executar( false );
} );

add_action( 'rest_api_init', function () {
	register_rest_route( 'robometria/v1', '/status', array(
		'methods'             => 'GET',
		'permission_callback' => '__return_true',
		'callback'            => function () {
			$e = robometria_sync_estado();
			return array( 'versao_sync' => ROBOMETRIA_SYNC_VERSAO, 'revisao' => $e['revisao'], 'ultimo' => $e['ultimo'], 'log' => array_slice( $e['log'], 0, 15 ) );
		},
	) );
	register_rest_route( 'robometria/v1', '/sync', array(
		'methods'             => 'POST',
		'permission_callback' => function () { return current_user_can( 'manage_options' ); },
		'callback'            => function ( $req ) {
			$e = robometria_sync_executar( (bool) $req->get_param( 'forcar' ) );
			return array( 'revisao' => $e['revisao'], 'ultimo' => $e['ultimo'], 'log' => array_slice( $e['log'], 0, 15 ), 'itens' => $e['itens'] );
		},
	) );
	register_rest_route( 'robometria/v1', '/token', array(
		'methods'             => 'GET',
		'permission_callback' => function () { return current_user_can( 'manage_options' ); },
		'callback'            => function () {
			return array( 'token' => robometria_sync_token(), 'url' => home_url( '/?robometria_sync=' . robometria_sync_token() ) );
		},
	) );
} );
