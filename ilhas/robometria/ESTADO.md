...
executando_desde: 2026-09-20T14:05Z
bloco_atual: |
  ...
  O DEFEITO: teste-casca.php reprovava 3 de 251. Os tres snippets tocados as 16h16Z subiram a CONSTANTE de versao e nao ganharam a entrada de cabecalho correspondente — casca 1.10.1 contra 1.11.0, R1 1.10.0 contra 1.11.0, R2 1.8.0 contra 1.9.0. Nada disso muda comportamento no ar: o que estava errado era o changelog, que e a primeira coisa que alguem le nesses arquivos. As tres entradas foram escritas com o que a execucao anterior de fato mudou, lido do diff e nao de memoria.
  E O DEFEITO MAIOR E DE METODO: A REGUA NAO TINHA FALHADO, ELA NAO TINHA RODADO. A secao 16 do teste-casca.php nasceu em 14/09 depois de a mesma drenagem acontecer em DOIS arquivos; hoje pegou TRES de uma vez. Cada execucao enumera de cabeca quais bancadas rodar e escreve a lista como prova — e lista escrita de cabeca esquece, com o agravante de que o esquecido nao aparece em lugar nenhum, porque o verde das outras ocupa o espaco do que faltou. Mesma familia do robometria_casca_categorias(), lista digitada ao lado de lista contada, um andar acima.
  ferramentas/bancada.py NAO TEM LISTA: varre a pasta e classifica pela convencao de nome. 38 portoes sem rede, 6 conferir-* so com --no-ar, ZERO arquivos sem convencao. Portao novo entra no dia em que e escrito e arquivo que nao casa com convencao nenhuma e DENUNCIADO em vez de ignorado.
  ELE COBRA AS DUAS METADES DO VEREDITO — codigo de saida e palavra impressa — e nomeia como INERTE o portao que imprime REPROVADO saindo com zero. A regua desse julgamento tem regua propria e roda SEMPRE, como portao zero: 21 afirmacoes sobre as linhas REAIS desta pasta. Ela nasceu medindo porque a PRIMEIRA versao denunciou tres portoes sadios, lendo '0 falha(s).' como reprovacao — a frase de aprovacao mais comum da ilha.
  PROVA DE PONTA A PONTA, e nao so o verde: quebrando de proposito o cabecalho da R2, a passada devolveu saida 1 e TRES portoes vermelhos — teste-casca mais duas baterias de mutacao cujo mundo sadio depende do arquivo. Restaurado, 38 de 38 verdes e saida 0.
  BANCADA COMPLETA DEPOIS DO SYNC: 44 portoes, 0 falha, com os SEIS conferir-* medindo o site no ar na revisao 68. Entre eles conferir-no-ar, conferir-frase-de-compra-no-ar, conferir-atribuicao-no-ar, conferir-canal-na-resposta, conferir-kits-no-ar e conferir-reservatorio-no-ar.
  O DESPACHO DA PURGA DE CACHE ESTAVA ABERTO DESCREVENDO ERRADO ESTA ILHA, e o git deu a data: a guarda do caminho vazio e os dois portoes entraram em fa7de02, 16/09 as 19h43Z, DOIS DIAS antes de o despacho ser escrito. A aquametria portou a funcao daqui e escreveu para si o portao que aqui ja existia. Medido hoje: teste-purga-cache 21 afirmacoes 0 falha, mutacoes-purga-cache 8 baterias 0 inertes. A metade da clubedomosaico continua aberta e foi reconferida por leitura: cdm_casca_esvaziar_pasta comeca direto no realpath, sem a recusa do vazio.
  ITENS 1 E 4 DO DESPACHO DA SENTINELA CONTINUAM ABERTOS, com a tentativa registrada pela 18.4 e o bloqueio medido nos TRES canais da Shopee: search_items 403, pdp/get_pc 403, e a pagina de busca em 200 servindo 156.338 bytes IDENTICOS byte a byte para 'roborock' e para 'xiaomi'. E um quarto canal caiu hoje: SHOPEE_APP_ID e SHOPEE_SECRET nao estavam no ambiente desta rodada, entao nem a regua otimista da Open API existia. Por isso nenhuma palavra-chave foi trocada: trocar chave sem chamar a busca antes e o que medir-palavras-chave.py nasceu para impedir.
  UMA FRASE FALSA DESFEITA NO PROMPT.md: 'nao ha bloco de prospeccao na fila desta ilha' e falso — o bloco 6 esta na fila desde que ela foi escrita, e o motivo do adiamento de 09/09 deixou de valer. O bloco 6 NAO foi aberto, e o motivo esta escrito: o despacho de 18/09 e mais novo que a leitura semanal e diz que a peca nao precisa ranquear, so estar alcancavel de dentro da R2.
