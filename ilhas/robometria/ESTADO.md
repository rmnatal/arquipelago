...
estado: nascendo
prioridade: 1
ultima_execucao: 2026-09-11T13:17Z
executando_desde: 2026-09-11T15:16Z
bloco_atual: "BLOCO DE DADOS ENTREGUE: as DUAS dividas nomeadas no PROMPT.md fechadas na mesma execucao, sem rede (testada antes: a ilha 200, mi.com.br e wap 000, entao o 3c segue bloqueado). (1) ACENTUACAO: 121 strings restauradas nos nove campos que a varredura mediu chegando a tela — a mesma frase da R1 servia 'a peca certa depende da sua' ao lado de 'vende a peca PR10127 identificada como Versao A', metade escrita pela ilha e metade citada do banco. A operacao e PROVADA diacritico-only: reduzido a sem-diacritico, o banco de hoje e byte a byte o de ontem, nos tres arquivos inteiros e depois linha a linha. E NAO e releitura — o acento foi reposto pela ilha, nao lido no fabricante, e por isso dados/acentuacao-restaurada.json existe como lista de conferencia de quem reler os manuais quando a rede abrir. Monossilabo ambiguo ficou fora do mapa e a borda das palavras inclui o hifen, para nao acentuar pedaco de URL citada. A primeira versao da trava reprovou 15 mudancas CERTAS: ela reduzia a ASCII e apagava o travessao que ja estava la. (2) A REGRA DO NIVEL 2, decidida: manual de fabricante guardado por terceiro e colhido por busca NAO e nivel 2, e nivel 3 — uma origem tem tres elos (autoria, custodia, leitura) e o nivel e o do MAIS FRACO. A direcao saiu da assimetria de custo: errar para baixo custa uma frase mais fraca, errar para cima faz a metodologia declarar um rigor que a ilha nao tem. A medicao decidiu sozinha — as QUATRO unicas fontes de nivel 2 do banco eram a MESMA entrada de manuals.plus. O QUE A CORRECAO DESCOBRIU SEM PROCURAR: a coluna Temos hoje era DIGITADA no snippet, e ao virar contagem do banco caiu um segundo numero que ninguem tinha visto — o nivel 6 dizia 'sim' e a ilha nao tem UMA fonte de marketplace. E a primeira versao do gerador quase trocou o erro por um pior, contando so dois arquivos e declarando que a ilha nao tinha fonte editorial, quando tem sete que a R2 cita em toda resposta. BANCADA: varrer-corpo.php monta os 72 estados que a ilha serve (a R1 modelo a modelo, a R2 situacao a situacao) — e montando-os no MESMO processo o cabecalho apareceu no primeiro e sumiu nos 71 seguintes, por um static legitimo da marca; um processo por estado devolveu 45 KB de corpo que sumiam em silencio. Terceira vez que esta ilha paga por bancada que serve menos que o site. TRAVAS: teste-acentuacao.php com 16 medicoes lendo o CORPO, regua escrita no proprio teste, e a excecao do wordmark declarada no markup e CONTADA uma a uma. SEIS mutacoes deliberadas e as seis reprovaram — a melhor e tirar a classe do wordmark, que faz ROBO ser pego 72 vezes e prova que a excecao e carregada. Em validar-banco.py, duas invariantes novas testadas quebrando o banco: origem tem que bater com o nivel na escada (era por ai que o defeito entrava), e nivel 1 ou 2 exige leitura declarada, porque silencio nunca promove. VERIFICACAO: casca 64, r1 90, a1 53, r2 86, a2 62, acentuacao 16, validar-banco aprovado; Chromium em 360/390/782/1200 nas nove paginas, 0 px de rolagem nas 36 medicoes, corpo de 1.886 a 18.505 caracteres. 44 itens esperando link (nao mudou; este bloco nao tocou em catalogo). ACHADO NA FILA, mesma familia e NAO corrigido: robometria_casca_numeros() le a option de cobertura-r1, que tem publicar=false — no ar a secao 4 da metodologia serve numeros DIGITADOS. Hoje eles por acaso batem com a medicao: latente, nao disparado, que e como a escada ficou meses errada. Proximo: derivar esses cinco numeros para casca-fatos.json, pelo cano que este bloco abriu"
