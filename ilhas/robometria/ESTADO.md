...
executando_desde: 2026-09-16T13:17Z
bloco_atual: |
  ...
  POR QUE E DEFEITO DE FAMILIA E NAO DESCUIDO: a lista digitada estava colada, na MESMA frase, a dois numeros computados do banco (71 pares, 5 marcas). Numero computado ao lado de lista digitada faz a lista parecer medida, e ninguem reconfere o que parece medido. Mesma familia do numero de tela digitado que esta ilha ja pagou duas vezes.
  ERAM TRES COPIAS, NAO DUAS: (1) o vocabulario no esquema, que e a fonte; (2) TIPOS_CONSULTAVEIS digitada dentro de cobertura-r1.py, sob um comentario que dizia 'vocabulario de tipo_de_peca do esquema, menos kit' — o arquivo DESCREVIA a derivacao e IMPLEMENTAVA uma copia; (3) a frase em portugues dentro do snippet. A copia 2 estava certa por sorte. A 3 nao.
  O CONSERTO: nasceu a chave tipos_consultaveis_na_r1 no esquema, e ela e REGRA DE DERIVACAO e nao segunda copia (derivado_de mais excluidos). A referencia deriva dela e falha alto quando some; a descricao sai do campo tipos, que ja era gerado da varredura.
  A BATERIA NOVA ACHOU DOIS BURACOS NA REGUA QUE ESTE BLOCO ACABARA DE ESCREVER, e os dois foram fechados antes do commit. Esta e a parte util. (a) A lista digitada mas CERTA hoje passava: texto digitado e texto derivado sao identicos enquanto o mundo fica parado, e so se separam movendo o mundo — a mutacao passou a fazer nascer um tipo E dar a ele a primeira peca, e ai a lista digitada envelhece na hora, exatamente como aconteceu em 13/09. (b) A copia dentro da referencia passava porque errava JUNTO com a frase: as duas saem da mesma varredura, entao tudo ficava internamente coerente com cinco faixas e nenhuma peca fora do banco. O teste passou a rederivar os tipos do esquema por um caminho que nao e o do gerador.
  DECISAO DE VOCABULARIO, aberta havia tres execucoes e agora fechada. 'tampa de escova' NASCE, junto com a coleta da primeira peca e nunca antes dela — tipo no vocabulario sem peca e promessa vazia esperando para ser prometida no JSON-LD, e a bateria tem uma mutacao so para esse caminho. 'saco descartavel' fica de fora, e a causa e de MERCADO e nao de vocabulario: saco so existe em robo com base de autoesvaziamento, e o banco declara base verdadeira em DOIS dos 38 modelos.
  E A CONTA QUE SUSTENTAVA A PERGUNTA ESTAVA ERRADA: os 'tres itens na mesma prateleira' eram DUAS funcoes somadas como se fossem uma — tampa de escova tem dois itens de uma marca, saco descartavel tem um de outra. Nenhuma das duas era faixa. O que destravou a decisao nao foi dado novo, foi o PRECO: acrescentar tipo passou a ser uma linha no esquema, e a mutacao 'MUNDO NOVO intacto — esta TEM de passar' mede que e mesmo uma linha.
  XIAOMI MOP 2 REMEDIDO, quatro passadas limpas restritas a mi.com, NEGATIVO: a Xiaomi publica pagina de acessorio POR FAMILIA e a familia Vacuum-Mop 2 (2021) nao tem uma. Nao e o canal, como na WAP — o canal responde, e responde que a pagina nao existe. O modelo fica no vazio da R1 com a causa escrita, que e dado e nao lacuna de procura.
  VERIFICACAO, 0 falha na bancada: teste-r1 223 (eram 216), teste-r2 107, teste-casca 205, teste-a1 68, teste-a2 74, teste-arvore 219, teste-voz 155, teste-acentuacao 17, teste-escada-compra 739, validar-banco aprovado, php -l limpo. NO AR: conferir-no-ar 229, 0 falha. MUTACOES: 20 baterias, todas no resultado esperado, ZERO inertes.
  PRONTO QUANDO, conferido no ar as 23h32Z e as 23h33Z em duas medicoes independentes: a descricao servida nomeia os SEIS tipos, com reservatorio incluido.
