...
bloco_atual: |
  ...
  ITENS 1 E 3 — A PALAVRA-CHAVE DA BUSCA PASSOU A SER MEDIDA, e a causa dos dois achados era uma so: a secao 26 ja dizia que o vocabulario da ilha classifica pela FUNCAO e o fabricante batiza pela POSICAO. A chave era composta do 'tipo' (escova principal, escova lateral), que e vocabulario da ILHA, e vendedor nenhum digita isso. Nasceu ferramentas/medir-palavras-chave.py: chama a Open API da Shopee e desce uma escada de cinco degraus — tipo, batismo do fabricante (a cabeca de nome_na_fonte), codigo da peca, codigo do modelo compativel, marca — e para no primeiro que serve. Das 18 chaves de ontem, TRES devolviam zero, e as tres eram 'escova principal'. Depois da escada: 20 de 20 com resultado e 16 de 20 no criterio do item 3, contra as 14 de 18 que o despacho pediu. Pela regua mais dura da 25.7 sao 7 de 20, e os dois numeros sao impressos lado a lado de proposito.
  O ESQUEMA PROMETIA ISTO POR ESCRITO DESDE 13/09 e a promessa venceu hoje; a MESMA frase supunha que codigo de peca nao aparece em titulo de anuncio, e medido e FALSO: loja de reposicao digita FW008024 e PR10205. A unica sem conserto e a bateria da Multilaser — o catalogo de ofertas nao a tem, e o topo e sempre o carregador.
  O QUE O DESPACHO NAO PEDIA E FOI JUNTO: o ENCURTAMENTO deixou de esperar alguem. generateShortLink com sub-id fechou o segundo elo do piso da 25.2, e 'saida crua, sem rastreio' foi de 5 para 0 em 73 publicaveis. E a regua achou um beco sem saida que despacho nenhum apontava, o modelo multi-ob010: 'Multilaser OB010 robo aspirador' devolve ZERO porque a Multi vende aquele aparelho como ObaDuster, da Obabox. E a secao 26 pelo outro lado — o codigo do modelo E o nome comercial, menos quando nao e. Quem pegou foi a varredura inteira, nao o caso-ancora.
  ITEM 2 — os dois universos do par ganharam NOME e nenhum foi igualado. A promessa serve 70 (as duas pontas publicaveis, mesmo universo dos 38 modelos que a frase cita) e os outros dois existem com nome proprio: pares_declarados_no_banco 71 e pares_para_modelo_nao_publicavel 1. mutacoes-universo-do-par.py prova que a regua reprova o mundo de ontem, 3 de 3.
  ITEM 4 — dateModified passou a sair de um CAMPO NOVO, medido do git sobre os arquivos que compoem a pagina, a casca inclusa. As outras duas opcoes foram descartadas com motivo: 'gerado_em' dos fatos mede a geracao dos FATOS e ficou cego para a revisao 52, que mexeu na casca sem regerar fato; a revisao do manifest e da ilha inteira. datePublished continua DECLARADO porque o historico do main foi reescrito e o git jura que tudo nasceu em 15/09. A pagina do A2 serve datePublished pela primeira vez.
