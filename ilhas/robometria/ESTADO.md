...
ultima_execucao: 2026-09-12T15:40Z
executando_desde: 2026-09-12T17:18Z
bloco_atual: "A PROCEDENCIA DA AREA POR CARGA CHEGA AO CARTAO DO A2 (snippet 1.2.0, manifest revisao 20, /status conferido as 15h30Z em UM disparo). O BLOCO COMECOU MEDINDO NO AR, que era a licao escrita pelo bloco anterior, e o que a pagina mostrou era PIOR do que este cabecalho previa. O cabecalho dizia "5 cartoes com ZERO procedencia" — verdade e metade. Os cinco cartoes diziam "O fabricante declara 166 m2 por carga", e a frase era FALSA: os cinco modelos que declaram area por carga declaram pela fonte f-loja, degrau 4 da escada (varejo-oficial-da-marca), cujo quem_declara e "pela loja oficial da marca". A pagina nao estava so calando a origem; estava CREDITANDO AO FABRICANTE O QUE A LOJA TRANSCREVEU. E ISSO E LITERALMENTE O QUE O BLOCO DA R2 PREVIU EM 11/09, na pagina irma: "no dia em que um Pa entrasse por loja oficial da marca, a pagina emprestaria calada a autoridade do fabricante a quem apenas transcreveu". La o defeito era latente e a frase digitada era verdade por coincidencia do banco; AQUI O DIA JA TINHA CHEGADO e ninguem tinha olhado. A diferenca entre as duas paginas nao era o codigo — era que uma foi lida como um leitor le e a outra nao. A METADE ESTRUTURAL: procedencia_do_pa() sabia derivar UM campo so, e o A2 decide pela AREA, nao pelo Pa — regra derivavel por um campo so e regra que a segunda pagina reescreve. Agora existe procedencia_do_campo() em cobertura-r2.py, com procedencia_do_pa/_da_cobertura/_da_autonomia por cima, e regerar a R2 devolve r2-respostas.json BYTE A BYTE IDENTICO — prova de que a refatoracao nao mudou valor nenhum. NA TELA: ressalva do degrau ("confira a embalagem"), porta de compra, e so depois "Como sabemos — loja oficial da marca, verificado em 09/09/2026 · fonte" com nofollow. O titulo da secao tambem era digitado e agora so nomeia um publicador QUANDO TODOS OS ITENS CONCORDAM. E lendo o cartao como leitor apareceu outra: a frase repetia o numero que ja era o titulo do cartao uma linha acima; virou "Area por carga declarada pela loja oficial da marca". VERIFICACAO: teste-a2 de 63 para 73 medicoes com REGUA PROPRIA (le esquema-banco.json e modelos-robo.json direto, nunca a2-fatos.json, escrito pelo mesmo gerador que preenche a procedencia), MEDICAO NO CORPO (recorta cada <li>; a secao de procedencia do artigo tambem fala em "fabricante", e medir a pagina toda aprovaria um cartao mudo) e A ENTRADA INTEIRA (os cinco). mutacoes-a2-procedencia.py: 15 deliberadas, 15 reprovadas — TRES passaram na primeira rodada e viraram trava nova: (1) a ressalva empurrada para depois do botao, porque o teste media que ela EXISTISSE e nunca ONDE; (2) a tabela rotulos_de_origem divergindo da escada no lado que o site consome; (3) a que NAO era trava faltando e sim BANCADA INCOMPLETA — "o degrau 4 sem ressalva" passou porque a mutacao rodava so o teste da pagina, e a trava existia desde 11/09 no validar-banco.py sem ser chamada. Mutacao no BANCO julgada so pelo teste da PAGINA mede metade do mundo. A diferenca deste arquivo para o irmao da R2 vale ser lida antes do proximo bloco: la "a atribuicao volta a ser digitada" era INERTE e precisava produzir o mundo do defeito; aqui esse mundo ja era o mundo. O banco mutado segue necessario para as duas travas LATENTES (a atribuicao da AUTONOMIA, hoje da mesma fonte nos cinco, e o titulo com degraus misturados) — trava latente sem mutacao que produza o mundo dela e trava nao medida. DE QUEBRA, E NAO ERA DESTE BLOCO: a bancada acusou casca manifest 1.4.0 / snippet 1.4.1. A ronda de hoje as 14h40Z (1ffbdd2) consertou o robometria_casca_numeros() lendo option de publicar=false — o defeito latente que este arquivo nomeava — e NAO TOCOU NO MANIFEST, nem versao nem sha. O Sync nao tinha como ver o arquivo novo: o conserto ficou uma hora commitado e invisivel, que e a secao 4 pelo caminho mais discreto de todos, porque nem no log aparecia. Entrou nesta revisao; atualizar-manifest.py acusou os dois descasamentos antes do commit. NO AR as 15h30Z: conferir-no-ar.py de 72 para 82 afirmacoes, 0 falha — as nove URLs em 200 e os cinco cartoes do A2 com procedencia datada, ressalva do degrau 4, atribuicao da loja oficial, porta de compra antes da fonte, ressalva antes da porta e link discreto com nofollow. A AFIRMACAO MAIS IMPORTANTE DA LISTA NOVA MEDE A AUSENCIA: a frase "O fabricante declara" sumiu dos cinco — trava que so confere o texto novo aprova uma pagina que sirva os dois. 44 itens esperando link de afiliado (nao mudou; este bloco nao tocou catalogo), nenhum com loja possivel hoje; na vitrine do A2, 5 de 5 esperam link. Pauta da secao 17: pauta.md ainda nao existe — 0 escritos, 0 na fila, 0 recusados. PROXIMO, medido neste bloco e nao lembrado: a secao "Exatamente no limiar" da propria R2 nomeia modelos e Pa e NAO cita origem — e o ultimo lugar das duas ferramentas onde um numero decide e a origem nao aparece, e as pecas ja existem. Junto com ele, um achado de receita que este bloco viu e NAO consertou para nao alargar o bloco: os cinco itens da vitrine do A2 viajam com afiliado.sub_id_2 = "R2", porque o campo e do MODELO no banco e as duas paginas o compartilham — no dia em que o primeiro link entrar, todo clique vindo do artigo sera contado como da ferramenta. O conserto nao e trocar o valor no banco (quebraria a R2): e cada gerador carimbar o proprio codigo. Os kits e o 3c seguem parados por REDE (politica de egresso, remedida em 11/09). Tudo que cria URL espera o reenvio do sitemap no Search Console, que e do Raphael."
