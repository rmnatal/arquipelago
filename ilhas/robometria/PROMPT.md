...
**ADENDO DE 16/09/2026, 16h07Z — A CHAMADA FOI FEITA DE VERDADE, E ELA ENSINOU DUAS COISAS.** A credencial foi preenchida e uma consulta real ao `productOfferV2` rodou da nuvem. Voltou com tudo: titulo, `imageUrl` do CDN da Shopee, `productLink` (`shopee.com.br/product/<shopId>/<itemId>`), `offerLink` ja encurtado em `s.shopee.com.br`, preco minimo e maximo, nome da loja e `ratingStar`. **A API esta viva e o caminho de rede esta aberto — isto nao e mais hipotese.** Duas descobertas da mesma medicao, e as duas mudam o que voce vai construir:

1. **PALAVRA-CHAVE LONGA DEVOLVE ZERO, E ZERO NAO E ERRO.** A busca por `Electrolux KPCEL01 kit performance` voltou **0 resultados**; a busca por `Xiaomi S10 robo aspirador` voltou 3. Ou seja: a chave que hoje esta em `afiliado.url_busca_produto` foi escrita para a busca do site da Shopee, nao para a API, e varias vao voltar vazias. Construa uma **escada de palavra-chave**, tentada em ordem e parando na primeira que der resultado util: (a) o codigo da peca sozinho; (b) marca + codigo; (c) marca + nome do modelo + tipo de peca; (d) a chave atual de `url_busca_produto`. **Registre em qual degrau cada item casou** — e dado de procedencia, exatamente como a escada de fontes do banco. Item que percorre a escada inteira e nao casa fica sem foto e sem ficha, com o motivo escrito: isso e medicao honesta, nao falha.

2. **`offerLink` SEM `subId` PERDE A ATRIBUICAO DA ILHA.** Os links que a API devolveu vieram **sem** sub-id. O banco ja carrega `afiliado.sub_id_1: "robometria"` em todo registro, e e esse campo que permite separar o que a ilha rendeu do que veio de qualquer outro lugar. **Toda chamada que gerar link tem de mandar o sub-id da ilha**, e o link gravado em `afiliado.url` precisa carrega-lo. Se a consulta `productOfferV2` nao aceitar sub-id, use `generateShortLink`, que aceita, e grave o link dela. **Link sem sub-id nao entra no banco** — e melhor ficar no `url_busca` de hoje, que ao menos ja tem o sub-id, do que trocar por um link mais bonito que nao sabe dizer de onde veio.
...
## DESPACHO DA SENTINELA — 16/09/2026 (ronda diária, 14h50Z, no Chrome do Raphael)

**Isto tem prioridade sobre a fila (seções 18.1 e 18.5).** Aplique antes de qualquer bloco, verifique pela seção 8, registre no `REGISTRO.md` como "despacho da Sentinela de 16/09 — item N cumprido", e apague daqui o item cumprido **só depois de abrir a URL no ar e conferir o "pronto quando"** (18.4).

**RECONFERÊNCIA DO QUE A RONDA ANTERIOR DEIXOU (regra 19.4c).** `dados/consertos.md` foi aberto antes de qualquer outra coisa. A ronda de 14/09 não consertou nada — os quatro achados dela eram todos da lista fechada 19.2 —, então **não havia conserto a reconferir**. O que esta ronda reconferiu foi o despacho de 14/09, no ar: **os itens 1, 2 e 3 estão CUMPRIDOS E CONFERIDOS e saíram deste arquivo** (18.4). Medido hoje: (1) o endereço canônico e o mesmo endereço com quebra de cache servem HTML idêntico, diferindo só pelo comentário `<!--Generated by Endurance Page Cache-->` no rodapé — nas 9 URLs; (2) a R1 chama cada contagem pelo seu nome, sem igualar grandezas diferentes; (3) a R2 na situação-âncora `liso|nao` serve *"o Mundo Conectado publica dois números: recomenda 3.000 Pa e escreve que até 1.500 Pa já basta (verificado em 09/09/2026)"*, com **uma** linha de procedência e sem oração quebrada. Os itens 4 e 5 daquele despacho são do Raphael e continuam abaixo.

**O QUE PASSOU NESTA RONDA, para a Fundação não refazer** (medido por `fetch` + `DOMParser` no navegador, portanto no HTML servido): 9 de 9 URLs do sitemap em **HTTP 200**; `/status` na **revisão 52**, igual à do `manifest.json`; **console sem uma mensagem**; **zero `&#038;` dentro de `<script>`**; JSON-LD presente e parseável nas 9; `meta name="description"` e cinco propriedades `og:` nas 9; canonical nas 9; nenhum `noindex` indevido; corpo nunca começa por metadado YAML; nenhuma imagem sem `alt` (a ilha não serve imagem); trilha da 16.3 nas 8 páginas que não são a home e **ausente na home**, que é o certo; `BreadcrumbList` nas mesmas 8; **zero página órfã** — a URL com menos links internos de entrada tem **4**, e o mínimo da 16.4f é 2; **zero link interno quebrado**; **nenhuma palavra da lista "Proibidas" do `VOZ.md`** em `<title>` nem no primeiro parágrafo de nenhuma das 9; **zero ocorrência de "em breve"**; e todos os botões de compra das 4 páginas de conteúdo saem por link encurtado com `rel="sponsored"` e `noopener` — 0 busca crua carimbada de patrocinada, 0 encurtado sem carimbo.

**R1 EXECUTADA COM ENTRADA REAL:** `electrolux-erb44` + `bateria` devolve *"Não localizamos declaração do fabricante de bateria para este modelo; não vamos supor"*, sem bloco de compra — igual à linha de base. **R2 EXECUTADA COM ENTRADA REAL:** 200 m², piso liso, sem animal, referência `electrolux-erb60` devolve limiar de 3.000 Pa e **6 modelos**. Os 6 foram **recontados na mão** a partir de `dados/modelos-robo.json`: publicáveis, com `pa_declarado >= 3000` **e** com `canal_brasileiro` preenchido são exatamente `xiaomi-s40`, `xiaomi-h40`, `xiaomi-s20`, `xiaomi-s40c`, `xiaomi-s10`, `xiaomi-e10` — bate. O grupo `sem_canal_brasileiro` está NOMEADO na resposta servida, com os 6 nomes e com os 15.000 Pa do S40 Pro.

**CORREÇÕES APLICADAS PELA SENTINELA NESTA RONDA: NENHUMA.** Os quatro achados abaixo caem todos na lista fechada 19.2, que vale mais que a 19.1. Nenhum é da 19.1.

---

### 1. QUATRO ITENS APONTAM O BOTÃO DE COMPRA PARA UMA BUSCA QUE DEVOLVE ZERO RESULTADO

É o "defeito silencioso" que a própria 25.4-b antecipou em uma frase — *"busca não esgota, mas muda de nome"* — e é a primeira vez que ele é medido nesta ilha.

**Os quatro, nomeados e não estimados:**

| item | palavra-chave gravada em `afiliado.url_busca_produto` | resultados hoje |
|---|---|---|
| `wap-escova-direita-w300` | `WAP escova lateral robo aspirador` | **0** |
| `wap-escova-esquerda-w300` | `WAP escova lateral robo aspirador` | **0** |
| `wap-escova-frontal-wsmart` | `WAP escova lateral robo aspirador` | **0** |
| `positivo-11206519` | `Positivo escova principal robo aspirador` | **0** |

Os quatro têm `afiliado.url_busca` encurtada e carimbada de patrocinada. Ou seja: **o leitor clica num link que rende comissão e cai numa busca vazia.**

**Conferido no ar, e não só no banco.** `https://robometria.com.br/qual-peca-serve-no-meu-robo-aspirador/?modelo=wap-w300&peca=escova%20lateral` serve as duas escovas do W300 com **dois botões "Ver ofertas na Shopee"**, e os dois vão para a busca de zero resultado. É a página cujo assunto inteiro é a saída de compra.

**Como foi medido, sem gastar clique de afiliado** (25.4 proíbe clicar no link encurtado): `fetch` de dentro de uma aba já em `shopee.com.br` para `https://shopee.com.br/api/v4/search/search_items?by=relevancy&keyword=<a palavra-chave>&limit=10&newest=0&order=desc&page_type=search&scenario=PAGE_GLOBAL_SEARCH&version=2`, lendo `total_count` e `items`. As duas palavras acima devolvem `total_count: 0` e lista vazia.

**Por que a Sentinela NÃO consertou:** trocar a palavra-chave é dado do banco e é escolha entre duas opções defensáveis (19.2), e aplicar a troca exigiria **gerar link de afiliado novo**, que a seção 12 proíbe expressamente à ronda diária. Remover o link e manter o produto deixaria 4 itens sem saída de compra, violando o piso da 25.2 e o item 1 da DEFINIÇÃO DE PRONTA.

**Pronto quando:** as quatro palavras-chave de `afiliado.url_busca_produto` desses quatro itens devolverem `total_count > 0` na chamada acima, o encurtamento correspondente estiver gravado em `afiliado.url_busca`, e a página `?modelo=wap-w300&peca=escova lateral` no ar levar a uma busca com resultado. Sugestão de direção, não de escolha: a WAP publica essas peças como "Escova Direita/Esquerda Para Robô Aspirador Robot W300" — a palavra "lateral" não aparece no título do fabricante, e é aí que a busca provavelmente se perde (o próprio banco já registra isso: *"Quem chama esta peça de escova lateral é a Robometria, pelo contraste do catálogo do próprio fabricante"*). A escolha da palavra é da Fundação.

---

### 2. A R1 PUBLICA "71 PARES" E "38 MODELOS" NA MESMA FRASE, E OS DOIS NÚMEROS VÊM DE UNIVERSOS DIFERENTES

É exatamente a mesma família dos dois 63 que o item 2 do despacho de 14/09 desmontou, um nível adiante — e ela voltou porque a régua criada lá recomputa as quatro contas mas não pergunta **de que universo** cada uma sai.

A frase servida hoje na `/qual-peca-serve-no-meu-robo-aspirador/` é: *"Hoje são 71 pares peça × modelo declarados, em 5 marcas, cobrindo 30 dos 38 modelos do banco."*

**Recontado na mão a partir de `dados/pecas.json` e `dados/modelos-robo.json`:**
- entradas de `compatibilidade` em peças publicáveis: **71**
- dessas, as que apontam para um modelo **publicável**: **70**
- a que sobra: `multi-pr10124` → `multi-ho401`, e `multi-ho401` tem `status: nao_publicavel`
- modelos publicáveis cobertos: **30**; modelos publicáveis sem nenhuma peça declarada: **8** (`multi-ho243`, `multi-ho411`, `positivo-pra500`, `wap-w1000`, `wap-w100c`, `wap-w310`, `wap-w400`, `xiaomi-mop-2`) — isso bate com a página

Ou seja: o **71** conta um par para um modelo que os **38** excluem e que **não está no seletor da ferramenta**. Nenhum leitor chega nele; nenhuma célula da tabela o responde. A frase é internamente incoerente e o erro é de um, que é o tamanho em que ninguém olha.

**Por que a Sentinela NÃO consertou:** decidir se "pares declarados" conta pares para modelo não publicável é escolher entre duas opções defensáveis, e o número sai de código, não de texto digitado (19.2 nos dois motivos). Igualar por igualar seria repetir o erro que o item 2 de 14/09 evitou.

**Pronto quando:** a frase no ar declarar um número de pares que sai do mesmo universo dos 38 — ou declarar os dois universos com nomes distintos, como a tabela já faz —, e `teste-r1.php` ganhar uma afirmação que reprove um par apontando para modelo não publicável contado dentro da promessa. A régua tem de reprovar o mundo de hoje antes de aprovar o de amanhã.

---

### 3. O PISO DA 25.2 ESTÁ DE PÉ E O TOPO DELE MANDA O LEITOR PARA OUTRO PRODUTO

Não é link morto e não trava nada — é qualidade do único degrau que esta ilha tem. Fica registrado porque **o topo é o espaço mais caro da página** e porque é o tipo de coisa que nunca aparece numa régua de HTTP 200.

Varredura das **18 palavras-chave de peça** — elas cobrem os **35 registros de peça publicáveis, 100% deles**. Das 18, **2 estão mortas** (item 1) e **16 vivas**. Dessas 16, em **9** o PRIMEIRO resultado não é a peça pedida, e em **6** é o mesmo anúncio: o robô **"Robô Aspirador e Passa Pano KABUM! smart 700"**.

Os casos, medidos:
- `Multilaser bateria robo aspirador` → 1º = *"Bateria 12,8V 1800mAh **Wap** Robot W100 FW00899"* — bateria de outra marca
- `Multilaser mop robo aspirador` → 1º = robô Kabum
- `Electrolux mop robo aspirador` → 1º = robô Kabum (a peça certa aparece em 3º)
- `Electrolux kit robo aspirador` → 1º = *"Aspirador De Pó Portátil Automotivo 70W"* (a peça certa em 3º); **esta palavra serve 5 itens**
- `Positivo escova lateral robo aspirador` → 1º = robô Kabum
- `Positivo mop robo aspirador` → 1º = robô Kabum
- `WAP reservatorio robo aspirador` → 1º = robô Kabum
- `Xiaomi escova lateral robo aspirador` → 1º = robô Kabum
- `Xiaomi mop robo aspirador` → 1º = *"Robô Xiaomi S40 PRO ORIGINAL - 2026"* — a marca bate, a peça não; e o S40 Pro é justamente um dos modelos que o portão do canal brasileiro tira da recomendação

As 7 que entregam a peça certa no topo têm uma coisa em comum e ela é a pista: a palavra-chave nomeia **marca + tipo de peça** num par que os vendedores realmente usam no título (`Electrolux filtro`, `Multilaser filtro`, `Xiaomi escova principal`). As que falham pedem um tipo que o vendedor escreve com outra palavra, ou nenhuma.

**Por que a Sentinela NÃO consertou:** mesmo motivo do item 1 — palavra-chave é dado do banco e o conserto passa por gerar link novo.

**Pronto quando:** em pelo menos **14 das 18** palavras-chave, o primeiro resultado da chamada de busca acima contiver a marca **e** o tipo de peça do registro; e existir uma régua na ilha que meça isso e imprima o número, para a próxima passada não precisar remedir na mão.

---

### 4. `dateModified` DOS DOIS ARTIGOS DIZ 13/09 E A PÁGINA MUDOU EM 16/09

Medido no JSON-LD servido: `/filtro-universal-de-robo-aspirador/` traz `dateModified: 2026-09-13` e `datePublished: 2026-09-10`; `/quantos-m2-o-robo-aspirador-limpa-por-carga/` traz `dateModified: 2026-09-13` e **nenhum** `datePublished`. As outras 7 páginas não publicam data nenhuma, o que é coerente com os tipos delas.

As duas páginas mudaram em 16/09 na revisão 52: o bloco de compra passou a sair por link encurtado com `rel="sponsored"`, e as contas da divulgação mudaram. `dateModified` está atrás do que o leitor e o Google recebem.

`dateModified` está na lista da 19.1 e a Sentinela consertaria — **mas a origem do campo o tira de lá**: ele é derivado de `gerado_em` dos arquivos `dados/a1-fatos.json` e `dados/a2-fatos.json`, que hoje valem `2026-09-13` e significam *a data em que os FATOS foram gerados*. Carimbar 16/09 ali seria mentir sobre a geração dos fatos para acertar a data da página. **Decidir qual campo alimenta `dateModified` — a geração dos fatos, a revisão do manifest, ou um campo novo — é escolher entre duas opções defensáveis: 19.2, despacho.**

**Pronto quando:** as duas páginas servirem `dateModified` igual à data da última revisão que mudou o HTML delas, `/quantos-m2-…/` também servir `datePublished`, e o `REGISTRO.md` dizer em uma linha de qual campo `dateModified` passou a sair.
