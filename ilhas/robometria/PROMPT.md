...
> **A PRIMEIRA LEVA SAIU EM 21/09/2026, 19h54Z — CINCO URLs NO AR, CONFERIDAS.** `/pecas/`, `/pecas/filtros/` e as três filhas de marca (`xiaomi`, `wap`, `electrolux`), snippet `robometria-malha.php` v1.0.0, manifest na revisão 80 e `/status` em 80. As cinco respondem 200, entraram no `wp-sitemap-posts-page-1.xml` (de 9 para 14 URLs) e são as primeiras páginas desta ilha com **pai de verdade** no WordPress (`post_parent`, 16.2) — as nove antigas moram todas na raiz.
>
> **O QUE A LEVA MUDOU NO NÚMERO QUE ESTE ARQUIVO TRAZIA, e a mudança é o próprio recontar que o `ARVORE.md` manda fazer:** contando também os **kits** que declaram filtro na composição — que é o que o leitor de fato compra para trocar o filtro do ERB80 —, a categoria tem **16 itens em 5 marcas** (11 filtros avulsos + 5 kits), e a Electrolux tem **6** itens, não "1 + 5". Nenhuma frase chama kit de filtro: onde há kit, o sujeito é "itens que entregam filtro" e a composição sai escrita.
>
> **A CLASSIFICAÇÃO DE SERP DA 14.9 FOI FEITA ANTES DE ESCREVER PÁGINA NENHUMA** e está em `dados/serp-malha-pecas-2026-09-21.md`, consulta a consulta, com o que ocupa o top 10 de cada uma. Nas cinco, o top 10 responde pelo **aparelho** e a pergunta da **peça** não tem resposta publicada; no `filtro do ERB60` são **10 de 10** resultados da ficha de venda do robô. O arquivo também escreve o que a medição **não** autoriza — a família "melhor robô aspirador" segue fora da fila.
>
> **A DECISÃO DE HIERARQUIA NO WORDPRESS, que este arquivo listava como pendente, foi tomada:** quem cria página de malha é o snippet dela, na ordem da árvore (mãe antes de filha, porque o `post_parent` precisa do id da mãe), e a identidade canônica de cada página é o meta `_robometria_id` com a **chave** (`pecas-filtros-xiaomi`) — nunca o `post_name`, que é só o último degrau e colidiria com `/modelos/xiaomi/` no dia em que ela nascer. A casca não ganhou um segundo mapa da árvore: ganhou `robometria_casca_malha()`, um registro filtrado que o snippet preenche.
>
> **A PRÓXIMA LEVA DESTA CATEGORIA JÁ ESTÁ MEDIDA, e ela não é de filtro:** `/pecas/escovas-laterais/` (13 itens, 5 marcas) e `/pecas/mops/` (19 itens, 5 marcas) **também passam no portão hoje**. Não nasceram porque a ordem de levas manda uma categoria por vez e o teto da 21.4 são 10 URLs por leva — não porque falte dado. Cada uma custa uma entrada no registro do `snippets/robometria-malha.php` mais a classificação de SERP das consultas novas. `/pecas/escovas-principais/` (só uma marca com 3 itens) e `/pecas/baterias/` (1 marca) seguem travadas por dado, e isso é o portão da 16.5 funcionando.
>
> **O QUE A LEVA NÃO FEZ, dito para ninguém somar errado:** o menu continua apontando "Peças" para a **ferramenta**, e não para `/pecas/`. O `ARVORE.md` condiciona a troca do rótulo do menu a `/pecas/` **e** `/succao/` existirem, e `/succao/` nasce na leva 2 — trocar metade do menu agora deixaria dois rótulos irmãos apontando para naturezas diferentes. `/ferramentas/` também continua de pé, pelo mesmo motivo: ela é retirada com 301 quando as duas seções existirem.

*(O texto que este quadro substituiu, de 21/09/2026 de manhã, está preservado abaixo — é a medição que autorizou a leva.)*
