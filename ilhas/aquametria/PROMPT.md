...
- **Feito: C3, C5 e C15.** A C15 saiu em 09/09/2026 às 21h (snippet v1.2.0, revisão 29) e levou junto as outras duas peças da seção 5 que faltavam nela — resposta antes da explicação e tabela de exemplos pré-renderizada.
- **Falta: C12 e C1, e depois os 3 artigos.** A C12 é a próxima, e o achado desta execução é que ela precisa do **mesmo pacote de três peças da C15, não só do JSON-LD**: medido em 09/09, nem a C12 nem a C1 servem tabela de exemplos pré-renderizada, e nenhuma das duas tem resposta direta no topo. Ou seja, o T7 nessas duas é maior do que "acrescentar schema" — planeje como bloco de calculadora, não como retoque. O molde pronto está na C15 (`aquametria_c15_casos_exemplo`, `_exemplo`, `_exemplos_html`, `_resposta_direta_html`, `_jsonld_dados`), e o caminho é espelhar em PHP as funções de arredondamento do próprio script da calculadora, senão a tabela servida contradiz a calculadora logo acima dela.
- **Cuidado que custou duas rodadas de teste na C15:** não reaproveite classe CSS que o teste de navegador usa como localizador (`.aqm-c12-fontes`, `.aqm-c12-aviso-afiliado` e irmãs). Tabela nova e nota nova pedem classe nova, com o estilo herdado pela lista de seletores.
