...
- **Feito: C3, C5, C15 e C12.** A C15 saiu em 09/09/2026 às 21h (snippet v1.2.0, revisão 29) e a **C12 em 09/09 às 23h17Z** (snippet v1.2.0, revisão 30), as duas com o pacote inteiro de três peças — resposta antes da explicação, tabela pré-renderizada e JSON-LD.
- **Falta: a C1, e depois os 3 artigos.** A C1 é a última calculadora sem as três peças, e o `conferir-entidades.mjs` a mostra como a única com `jsonld_ok=0`. **Planeje como bloco de calculadora, não como retoque de schema**: ela não serve tabela de exemplos nem tem resposta direta no topo. Agora existem DOIS moldes prontos (C15 e C12), e o caminho é o mesmo — espelhar em PHP as funções de arredondamento do próprio script (`mL`, `litros`, `pct`, e as que montam as âncoras), senão a tabela servida contradiz a calculadora logo acima dela.
- **O teste já está preparado para receber a C1:** `ferramentas/teste-navegador-visibilidade-ia.mjs` mede as quatro que já têm as peças (124 afirmações, JavaScript DESLIGADO). Para a C1, basta acrescentar uma entrada em `CASOS` com o `eixo` e a `ancora` dela. O eixo é configurável de propósito: C3, C5 e C12 respondem por litro, a C15 por centímetro, e cravar litro para todas reprovaria a C15 por estar certa.
