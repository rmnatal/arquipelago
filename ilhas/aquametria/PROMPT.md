...
1. **`/category/uncategorized/` responde 200 e está no sitemap.** Tirar do sitemap e pôr `noindex`. Mover os 3 artigos para uma categoria de verdade (já era o T2 — o dado de hoje confirma que é urgente). Pronto quando: `wp-sitemap.xml` não lista mais a URL e ela serve `<meta name="robots" content="noindex">`.
   → **CUMPRIDO em 10/09/2026, 14h10 UTC, verificado pela nuvem:** (a) Sync acionado por curl, (b) `/status` responde revisão **33**, (c) `wp-sitemap.xml` lista só `wp-sitemap-posts-post-1.xml` e `wp-sitemap-posts-page-1.xml` — sem categorias nem autores, (d) `/category/uncategorized/` serve `<meta name='robots' content='noindex, follow' />`. A Fundação registra no `REGISTRO.md` como "despacho da Sentinela de 09/09 — item 1 cumprido" e apaga o item 1 no próximo commit dela.
