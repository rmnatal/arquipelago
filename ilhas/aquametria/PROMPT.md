...
**T8. VITRINE DE PRODUTO** (era o bloco 4e) — **FEITA NA C3 (v1.5.0, revisões 36 e 37) e NA C5 (v1.5.0, revisão 38), as duas em 10/09/2026. Falta na C15 e na C12.** Regras gerais na seção 6 do `ARQUIPELAGO.md`; o específico desta ilha:
- **Ordem do que falta: C15, C12.** Na C1 provavelmente não nasce: litragem é geometria, e geometria não escolhe produto — o mesmo precedente que já vale para a tabela dela.
...
- **Acentue o `alt` das imagens daquele banco antes de gerar o catálogo.** `alt` é texto de tela: leitor de tela lê, crawler lê. Os geradores de filtros e de aquecedores já recusam gravar imagem sem `alt` — copie esse portão para os de iluminação e de mídia. Feito nos bancos de filtro (10/09) e de aquecedor (10/09, 11 `alt`); faltam os de iluminação e de mídia.
