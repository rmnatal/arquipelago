 ...
 * Versão: 1.10.0 (15/09/2026) — A ILHA PASSA A PURGAR O CACHE DO HOSPEDEIRO, e
 * com isso fecha o despacho de prioridade ALTA da Fundação aberto em 14/09/2026
 * em `dados/despachos.md`, que dizia com todas as letras "falta a aquametria".
 * O que a robometria pagou para descobrir: o Sync grava OPTIONS e atualiza
 * SNIPPETS, nunca salva post no wp-admin, e é o POST salvo que purga o cache em
 * arquivo do hospedeiro — então o `/status` afirma que a revisão está aplicada
 * enquanto o leitor continua na página de antes, por até duas horas.
 * O QUE O DESPACHO SUPUNHA SOBRE ESTA ILHA ESTAVA MEIO CERTO, e a metade errada
 * é a que interessa: ele registrou que a home "não traz a assinatura" do
 * Endurance Page Cache e que a ilha não tinha sido medida página por página.
 * Medido agora nas 40 URLs do sitemap, com cabeçalho e corpo na MESMA requisição
 * como a seção 4 do contrato exige: a assinatura realmente não aparece no HTML —
 * e `x-server-cache: true` aparece nas 40, com `x-proxy-cache` e
 * `cache-control: max-age=7200`. O cache sempre esteve lá; o que faltava era uma
 * régua que olhasse para o cabeçalho em vez de procurar um comentário no corpo.
 * A purga é `aquametria_casca_purgar_cache()`, na seção 12 deste arquivo, com o
 * gatilho na gravação de qualquer option `aquametria_*`. Quem a mede são
 * `ferramentas/conferir-cache-do-host.py` (no ar, canônico contra quebra de
 * cache) e `ferramentas/teste-purga-cache.php` (na bancada, as guardas do único
 * código desta ilha que apaga arquivo) — e a segunda achou um buraco de verdade
 * no primeiro minuto de vida, descrito na própria seção 12.
 *
...
/**
 * SEÇÃO 12 — A PURGA DO CACHE DO HOSPEDEIRO.
 *
 * DESPACHO DE PRIORIDADE ALTA DA FUNDAÇÃO, aberto em 14/09/2026 em
 * `dados/despachos.md` depois do achado da robometria, e esta ilha era a que
 * faltava. Lá, a revisão 37 aplicou, o `/status` respondeu 37, as nove URLs
 * deram 200 — e o endereço canônico serviu por cerca de hora e meia a cópia
 * anterior, sem um botão de compra e com uma frase que o contrato acabara de
 * proibir. Sync aplicado e `/status` batendo NÃO são entrega (seção 4 do
 * `ARQUIPELAGO.md`).
 *
 * O QUE ESTA ILHA MEDIU, e é o motivo de a porta se fechar aqui também. O
 * despacho registrou que a home da aquametria "não traz a assinatura" do
 * Endurance Page Cache e que não tinha sido medida página por página. Medido em
 * 15/09/2026 às 11h18Z, nas 40 URLs do sitemap, com cabeçalho e corpo na MESMA
 * requisição (`curl -D - -o`, como a seção 4 exige): o comentário
 * `<!--Generated by Endurance Page Cache-->` realmente NÃO aparece no HTML —
 * **e os cabeçalhos nomeiam as duas camadas assim mesmo**: `x-server-cache:
 * true` nas 40, `x-proxy-cache` com HIT/MISS, e `cache-control: max-age=7200`.
 * Ou seja: a ausência da assinatura no corpo nunca foi prova de ausência do
 * cache — ela era só a régua olhando para o lugar errado. A janela de duas
 * horas desta ilha é a mesma da robometria.
 *
 * POR QUE A PURGA MORA NA CASCA, e não no Sync, que seria o lugar óbvio: o Sync
 * se pula a si mesmo por desenho, então correção nele não chega ao site por ele
 * mesmo. A casca é publicada a cada revisão e já está carregada durante a
 * requisição do Sync — o gatilho é a própria gravação das options da ilha.
 *
 * E A CONSEQUÊNCIA QUE A CLUBEDOMOSAICO MEDIU E QUE VALE PARA TODA ILHA: a
 * purga só passa a valer no SEGUNDO Sync. No Sync que INSTALA esta casca, o PHP
 * já carregado é o anterior, e por isso nenhuma purga roda. Quem publicar este
 * arquivo dispara o Sync duas vezes.
 *
 * A LISTA É DEFENSIVA DE PROPÓSITO. A ilha não escolhe o hospedeiro e não pode
 * medir daqui qual cache estará instalado amanhã; cada entrada é um `if` sobre
 * algo que pode não existir, e nenhuma delas é obrigatória. O que NÃO é
 * defensivo é a verificação: quem confere é
 * `ferramentas/conferir-cache-do-host.py`, que põe o endereço canônico contra o
 * mesmo endereço com quebra de cache e reprova quando os dois discordam. Purga
 * que ninguém mede é purga que envelhece calada.
 */
if ( ! function_exists( 'aquametria_casca_purgar_cache' ) ) {
function aquametria_casca_purgar_cache() {
	static $ja = false;
	if ( $ja ) {
		return;
	}
	...
	/* Endurance Page Cache — é o que o `x-server-cache: true` desta hospedagem
	   anuncia. As duas formas: a ação que as versões novas escutam e o método
	   da classe, para as antigas. */
	do_action( 'epc_purge' );
	...
	/* A PURGA QUE NÃO DEPENDE DO NOME DO GANCHO: esvaziar
	   `wp-content/endurance-page-cache/`, que é o que o `purge_all()` do próprio
	   plugin faz. Ela existe porque o gancho pode ter mudado de nome entre
	   versões, e um gancho que ninguém escuta falha em silêncio.

	   AS GUARDAS, e nenhuma é decoração — este é o único código desta ilha que
	   apaga arquivo: o caminho é montado a partir de WP_CONTENT_DIR e nunca de
	   entrada de requisição; o caminho real é conferido contra a raiz a cada
	   nível da recursão; link simbólico não é seguido; e caminho VAZIO é
	   recusado antes de tudo. Quem mede as guardas é
	   `ferramentas/teste-purga-cache.php`. */
	$pasta = defined( 'WP_CONTENT_DIR' ) ? WP_CONTENT_DIR . '/endurance-page-cache' : '';
	if ( '' !== $pasta && is_dir( $pasta ) && ! is_link( $pasta ) ) {
		aquametria_casca_esvaziar_pasta( $pasta, $pasta );
	}
 ...
 * com ela.
 *
 * A PRIMEIRA LINHA DESTA FUNÇÃO FOI ESCRITA DEPOIS DE ELA APAGAR A ILHA INTEIRA,
 * em 15/09/2026, e o registro fica porque a causa é uma armadilha da linguagem
 * que nenhuma leitura do código revelaria. **`realpath('')` não devolve `false`
 * em PHP: devolve o DIRETÓRIO DE TRABALHO ATUAL.** Então, chamada com caminho e
 * raiz vazios, a guarda de contenção comparava o diretório atual consigo mesmo,
 * aprovava, e a recursão esvaziava o diretório de trabalho — que na bancada é a
 * pasta da ilha. Foram 117 arquivos apagados, recuperados na hora com
 * `git checkout` porque o repositório é o lugar do trabalho (seção 3); num
 * servidor não haveria de onde recuperar.
 *
 * O QUE ISSO DIZ SOBRE A GUARDA, e é maior do que o caso: ela era escrita como
 * "o caminho real tem de estar DENTRO da raiz real", o que é a pergunta certa —
 * e `realpath()` respondeu por um caminho que nunca foi pedido. Guarda que
 * confia numa função de biblioteca para dizer "isto não existe" herda todos os
 * valores que essa função devolve quando a entrada é degenerada. A correção é
 * uma linha e não substitui a guarda: ela recusa o vazio ANTES, para que a
 * comparação nunca chegue a acontecer sobre um caminho inventado pela
 * linguagem.
 *
 * NO SITE NADA DISSO ACONTECIA: o único chamador monta o caminho a partir de
 * `WP_CONTENT_DIR` e já exigia `'' !== $pasta && is_dir( $pasta )`. O defeito
 * era da FUNÇÃO, não do caminho de produção — e é exatamente o tipo de defeito
 * que só aparece quando alguém escreve o portão, que é o motivo de o portão
 * existir. A robometria e a clubedomosaico publicaram esta mesma função sem
 * portão e sem esta linha; virou despacho em `dados/despachos.md`, porque a
 * seção 3 proíbe editar ilha que não se reservou.
 */
if ( ! function_exists( 'aquametria_casca_esvaziar_pasta' ) ) {
function aquametria_casca_esvaziar_pasta( $pasta, $raiz ) {
	...
	foreach ( $itens as $item ) {
		...
		if ( is_link( $caminho ) ) {
			...
			aquametria_casca_esvaziar_pasta( $caminho, $real_raiz );
			@rmdir( $caminho );
		} else {
			@unlink( $caminho );
		}
 ...
 *
 * O Sync escreve `aquametria_dados_*` e `aquametria_sync_estado` a cada
 * aplicação, e a casca escreve `aquametria_casca_*` quando remonta a estrutura,
 * então uma revisão que muda QUALQUER coisa passa por aqui. O prefixo é
 * conferido para a purga não disparar em gravação de option de terceiro — e o
 * `static` da função acima faz a purga acontecer UMA vez por requisição, mesmo
 * quando a revisão grava vinte options.
 */
if ( ! function_exists( 'aquametria_casca_purgar_ao_gravar' ) ) {
function aquametria_casca_purgar_ao_gravar( $opcao ) {
	if ( 0 === strpos( (string) $opcao, 'aquametria_' ) ) {
		aquametria_casca_purgar_cache();
	}
}
}
add_action( 'updated_option', 'aquametria_casca_purgar_ao_gravar', 10, 1 );
add_action( 'added_option', 'aquametria_casca_purgar_ao_gravar', 10, 1 );
