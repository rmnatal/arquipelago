...
- PRÓXIMO: BLOCO 4 (continua) — **C12 mídia filtrante é a próxima**, pareada com o artigo-âncora dela. ATENÇÃO ao que o validador já mostra: os 3 filtros com `volume_filtragem_L` faltando (Atman AT-3338, AT-3338S e Seachem Tidal 55) barram a sugestão da C12, e o banco de mídia tem só 2 registros — o caminho é o mesmo das duas últimas execuções: **coletar com fonte ANTES de escrever a calculadora**, porque é a coleta que decide se o bloco de produto nasce cheio ou vazio. A C12 já tem duas âncoras de fabricante que conflitam por 2x (Seachem Matrix: 1,25 mL/L numa leitura da copy e 2,6 mL/L noutra) mais os 12 mL/L de cesto do Eheim 2213 — é o vácuo de conteúdo nº 1 do levantamento. Ordem restante: C12 mídia → C15 iluminação → C2 peso e carga → C7 consumo → C8 lotação. Cada uma = snippet PHP com shortcode (formulário + cálculo + resposta em faixa com critério e fonte, JS e CSS inline, sem dependência externa) + página em `conteudo/` + o artigo-âncora pareado + itens no manifest com `publicar: true` e sha256 correto + o snippet se anunciando no hub pelo filtro `aquametria_calculadoras` (NÃO se edita a casca para isso) + gerador de catálogo próprio, se tiver bloco de produto. A C1 é o estado compartilhado: chave `aquametria.aquario`, e a mescla é obrigatória — nenhuma calculadora apaga o que a outra gravou.
