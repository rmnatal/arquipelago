...
## 3. `/peixes/` — NO AR desde 12/09/2026 (levas 1 a 9 do T4)

A camada que a Bússola verificou ABERTA e a de maior volume de busca da ilha ("quantos litros para N neons"). O banco de espécies (`dados/especies-agua-doce.json`, **40 registros**, e os dois números derivados abaixo também foram RECONTADOS em 23/09/2026 pela leva 9, rodando `ferramentas/validar-especies.py`: são **34 de 40** no portão do catálogo e **34 de 40** no portão de página, com os mesmos 6 barrados nos dois. *(O que estava escrito aqui até hoje, e fica registrado porque é a doença que este documento mais pega: dizia **39 registros**, contados pelo Pente Fino em 21/09/2026, com a ressalva de que os derivados — '29 dos 37 passam', '14 dos 36 registros' — NÃO tinham sido recalculados e valiam como não verificados. Eram três números para um banco só. A leva 6 do banco, em 23/09, levou o arquivo a 40 registros, e a leva 9 recontou as três coisas de uma vez, com a ferramenta e não de cabeça.)* O banco é o que limita quantas filhas cabem, e desde 12/09/2026 são **duas** réguas com dois nomes, não uma:

- **`catalogo-de-especies`** — sete campos e duas fontes de corpos distintos. **34 dos 40 passam**, recontado em 23/09/2026 pela leva 9 com `validar-especies.py` (dizia "29 dos 37", de 13/09/2026). É quem entra na contagem da seção, na tabela da categoria e na lista de quem divide a mesma água.
- **`pagina-especie`** — o catálogo MAIS `cardume_minimo OU convivencia igual a solitario/casal/harem`. **34 dos 34 do catálogo passam**, recontado em 23/09/2026 pela leva 9 (dizia "29 dos 29", de 13/09/2026) — os seis barrados são os mesmos dos dois portões, e hoje ninguém está no catálogo sem poder ter página. É quem pode ter página própria, porque a ficha deste eixo se chama "quantos litros para um cardume de X" e abre pela frase que nomeia o cardume mínimo.
...
| `/peixes/barbos/` | barbo sumatra · barbo rosado · barbo cereja | **NO AR desde 23/09/2026 (leva 9, 4 URLs)** — os 3 registros que o nome **barbo** alcança neste banco, e é a **quarta categoria seguida em que o critério é outro**: aqui volta o NOME, que a leva 7 usou e a leva 8 teve de abandonar, e ele volta **contado** — `barbo` casa em exatamente três registros de `nomes_populares_br` no banco de 40. O que ele não suporta é ser encurtado: `barb`, a raiz do gênero antigo que o banco guarda nos sinônimos, casa em QUATRO, e o quarto é o `cascudo-barbudo`, que é vendido na prateleira dos limpa-vidros e está barrado pelo portão. Nem a família nem o gênero servem — Cyprinidae tem quatro elegíveis e o quarto é o kinguio, de 48 cm, e os três desta linha estão em três gêneros. É a **segunda categoria seguida em que a coluna que decide é a temperatura**, e a primeira em que ela ORDENA as três linhas em vez de separar duas: 18 a 22 °C (rosado), 20 a 26 °C (sumatra) e 23 a 27 °C (cereja) — os dois extremos não se cruzam em um grau e o do meio alcança os dois. Ao contrário dos acarás, os três portes estão na MESMA régua (5,0 · 7,0 · 14,0 cm TL). A ficha do **barbo sumatra** é a **primeira do eixo com restrição de companhia declarada pela fonte** (`restricoes_de_companhia`, esquema versão 6): a base científica manda não o manter com peixe de nadadeira longa, e a tabela de quem divide a água não tem campo para isso — então a página publica a recusa antes da tabela e diz o que a tabela não peneira. O mesmo campo consertou um defeito que estava NO AR havia nove dias na ficha do papilocromis. SERP das quatro consultas classificada em 23/09/2026, e as quatro páginas declaram `serp_em` |
