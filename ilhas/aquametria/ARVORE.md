...
A camada que a Bússola verificou ABERTA e a de maior volume de busca da ilha ("quantos litros para N neons"). O banco de espécies (`dados/especies-agua-doce.json`, 37 registros) é o que limita quantas filhas cabem, e desde 12/09/2026 são **duas** réguas com dois nomes, não uma:
...
| `/peixes/bettas/` | — | **banco fechado em 13/09/2026 — 3 espécies elegíveis**: betta (`solitario`), colisa-anão (`casal`) e gurami mel (`grupo`, cardume 4 a 6). É o mínimo exato do 16.5, sem folga. Os dois guramis grandes do banco — pérola e tricogaster — estão fora por falta de `convivencia`, medida em quatro passadas e dois corpos, não por falta de tentativa. **A classificação de SERP da 14.9 foi feita em 13/09/2026 e o critério foi escrito** (snippet 1.3.0): as quatro consultas são ALVO, e a categoria já declara o critério com a lista de espécies ainda vazia. Falta só a leva — e ela só pode nascer na semana que vem, pelo teto de 3 levas por semana da 21.4, gasto em 12/09 |
...
| `/peixes/vivaparos/` | — | **banco fechado em 13/09/2026 — 3 espécies elegíveis**: platy (`harem`), espada (`harem`) e **plati variatus** (`harem`), que entrou no banco na execução das 21h21Z e é quem levou a categoria de 2 para 3. É o mínimo exato do 16.5, sem folga, igual à `bettas`. **O critério foi escrito e a classificação de SERP da 14.9 foi feita em 13/09/2026** (snippet 1.5.0), com a lista de espécies ainda vazia — falta só a leva. **As três são do gênero `Xiphophorus`, e os dois vivíparos MAIS vendidos do Brasil estão fora**, cada um por um campo: o guppy tem duas urls de um corpo só (E15, espelho não confere espelho) e o molly não tem `comprimento_minimo_aquario_cm`. **A parede do molly mudou de natureza em 13/09/2026 e isso importa para quem for tentar de novo:** até então o que se sabia era que o compêndio publica a seção de dimensões dele vazia, e a saída óbvia era perguntar à base científica, que é quem sustenta esse mesmo campo para o platy e para o variatus. Foi perguntado, em duas passadas limpas, e a base **não** entregou a seção de aquário desta espécie — devolveu, nas duas, o número da espécie vizinha. Então os dois corpos já foram perguntados, e o desbloqueio dele passou a depender da leitura direta, que é o despacho de egresso aberto para o Raphael |
