...
- **Open API da Shopee** — `AppID` e `Senha` vazios; acesso não concedido. **PEDIDO ENVIADO PELO RAPHAEL EM 13/09/2026**, pelo formulário do Centro de Ajuda do Afiliado (`help.shopee.com.br/portal/10` → Acesso rápido → E-mail). O formulário tem **opção própria para solicitar API** na lista de temas — não é preciso descrever o pedido por extenso. Prazo declarado por eles: até 3 dias úteis para retorno. Ela é **GraphQL**, não REST: quando a credencial chegar, a consulta é um POST com `{"query": "..."}` no corpo, e não é preciso biblioteca nenhuma — `requests` basta, como em `ferramentas/ga4.py`. **Enquanto não chegar, nada depende dela.**
  **QUANDO A CREDENCIAL CHEGAR, a regra de credencial da seção 3 vale inteira e sem exceção:** `AppID` e `Senha` entram como **variáveis de ambiente da rotina**, do mesmo jeito que `GOOGLE_SA_B64`. **Nunca** em `ESTADO.md`, nunca em arquivo do repositório, nunca em log, nunca em saída de teste. Quem for escrever a ferramenta segue o molde de `ferramentas/ga4.py`, que já lê credencial de ambiente e falha com mensagem clara quando ela não está lá.
