...
A seção 1.2 diz que uma ilha por vez recebe construção. **Esta seção diz o que acontece com as outras**, porque "foco único" lido pela metade vira abandono, e abandono é como se perde um arquipélago inteiro construindo um site de cada vez.

**A REGRA, na formulação do Raphael em 18/09/2026:** *"dar foco primeiro em um projeto e ir para os próximos, sempre mensurando os anteriores, para o Arquipélago inteiro crescer junto como projeto geral."*

**1.2-b.1 — DOIS MODOS, E TODA ILHA VIVA ESTÁ EM UM DELES.** **Construção**: a ilha em foco, e só ela, recebe execução da Fundação. **Medição**: todas as outras. Na medição a Sentinela continua rodando, a série de `dados/indexacao.md` e `dados/posicoes.md` continua crescendo, e a ilha continua no ar ganhando idade de domínio — que é o ativo que não se compra. **Ilha fora do foco não para; ela deixa de consumir execução de construção e passa a produzir número.**

**1.2-b.2 — A MEDIÇÃO DAS ANTERIORES É O QUE FAZ A PRÓXIMA CUSTAR MENOS.** Isto não é burocracia, é o mecanismo do composto. Exemplo medido nesta mesma semana: a Robometria e a Aquametria foram medidas na mesma régua de volume absoluto (`bussola/medicoes/volume-absoluto-2026-09-18.md`), e o resultado virou o **piso de demanda da seção 3.0-b da `BUSSOLA.md`** — uma régua que nenhuma das duas tinha quando nasceu, e que a terceira ilha herda pronta. **Cada ilha medida barateia a seguinte. Ilha abandonada não ensina nada.**

**1.2-b.3 — A ORDEM DO FOCO SAI DA MEDIÇÃO, NÃO DA ROTAÇÃO NEM DA IDADE.** A fila de foco é reordenada por número, não por quem esperou mais. Em 18/09/2026 a Aquametria vem antes das outras três **porque é a única com demanda medida** — 51% das consultas do corpus com volume, e as paramétricas entre elas —, não por ser a mais velha. Ilha sem demanda medida não recebe foco antes de ilha com demanda medida, por mais avançada que esteja.

**1.2-b.4 — SAIR DO FOCO EXIGE DECISÃO ESCRITA, E ELA TEM TRÊS DESFECHOS.** Ilha que termina o foco não fica no limbo: recebe, no `PROMPT.md` dela, um critério **pré-registrado** — qual série, quais datas, e os três desfechos possíveis (volta ao foco, fica em manutenção, ou é arquivada com o aprendizado indo para a `BUSSOLA.md`). O modelo é o da Robometria, escrito em 18/09 antes de qualquer número chegar. **Critério escrito depois do dado é critério dobrado para caber no dado que veio.**

**1.2-b.5 — RESERVA ÓRFÃ É LIXO, E A PRÓXIMA EXECUÇÃO LIMPA.** Descoberto em 18/09/2026: `clubedomosaico` e `ohmetria` estavam com `executando_desde` preenchido desde 15/09 às 13h19 e 13h22 — três minutos de diferença, a assinatura de duas execuções que morreram juntas quando o limite semanal derrubou seis. **Duas das cinco ilhas ficaram invisíveis para a Fundação por três dias e nada detectou.** A regra: `executando_desde` com **mais de 24 horas** e **sem nenhum commit na pasta daquela ilha** desde então é reserva morta. A execução que encontrar limpa o campo, escreve uma linha dizendo que limpou, e segue — não é preciso pedir nada a ninguém. Reserva viva se prova por commit, não por campo preenchido.
...
**COMMIT DE QUEM? A 1.1 NÃO DIZIA, E O SILÊNCIO DELA TRANCA A ILHA QUE MAIS PRECISA DE TRABALHO (13/09/2026).** A regra acima manda descartar a ilha com commit na pasta nos últimos 40 minutos, e não diz **quem** fez o commit. Lida ao pé da letra ela descarta a ilha que a **Sentinela** acabou de tocar — e a ronda da Sentinela termina escrevendo um **despacho para a Fundação**, com prioridade sobre a fila pela 18.1. Ou seja: a letra da 1.1 manda a Fundação ignorar por 40 minutos exatamente a ilha que acabou de receber ordem de serviço, e a 18.1 manda o contrário. Aconteceu em 13/09/2026 às 15h16Z, com a aquametria: `executando_desde: null`, ronda fechada às 14h47Z, commit dela às 14h52Z, e um despacho novo de quatro itens no `PROMPT.md`.

**A regra, e ela cabe numa frase:** o que a 1.1 mede é **execução da Fundação viva**, então o commit que torna a ilha VIVA é **commit de bloco da Fundação**. Commit de ronda da Sentinela, de correção da 19, ou qualquer commit que não seja de um bloco em andamento, **não reserva nada** — e a prova de que nenhuma execução da Fundação está viva é mais forte do que qualquer relógio: **a reserva é escrita ANTES do trabalho (passo 5), então `executando_desde: null` já significa que não há bloco em andamento.** O git só é consultado quando há reserva VENCIDA para decidir se ela foi abandonada; com `executando_desde: null` não há o que desempatar, e a ilha está livre.
...
**O ENDEREÇO QUE VOCÊ TESTA SAI DA PROSA; O ENDEREÇO QUE VALE ESTÁ NO BANCO (Robometria, 13/09/2026).** A regra acima manda retestar o bloqueio de rede antes de respeitá-lo, e falta a pergunta anterior a essa: **retestar o quê?** A Robometria tinha três dias de coleta parada com "a rede está fechada" escrito no `PROMPT.md` e no `ESTADO.md`, e as duas execuções que retestaram obedeceram ao contrato — remediram, duas passadas, `000` nas duas, egresso confirmado fechado por política. Só que o domínio que a prosa mandava testar (`mi.com.br`, `xiaomi.com.br`) **não é o que o banco cita**: os sete registros da marca apontam para `www.mi.com/br` desde o primeiro dia, e a busca alcança esse domínio. O bloqueio era verdadeiro e irrelevante. Portanto: **o endereço de um teste de canal se lê do BANCO — do campo `url` da fonte que se quer reler —, nunca da frase que descreve o bloqueio.** A prosa é resumo de quem já falhou uma vez; o banco é onde a procedência mora, e ele não envelhece junto com a descrição do problema. Vale para toda ilha e todo canal: quem vai reabrir uma coleta bloqueada abre antes o registro que quer melhorar e copia o endereço de lá.
...
**E EXISTE UM TERCEIRO CACHE, DEPOIS DO SYNC, QUE NENHUMA REGRA DESTA SEÇÃO ALCANÇAVA — o do HOSPEDEIRO (Robometria, 14/09/2026).** Tudo acima trata do caminho até o site: o `raw` que serve velho, o `/status` que prova que a revisão chegou. **Nada disso diz que o LEITOR recebeu.** Medido na Robometria: a revisão 37 foi aplicada, o log do Sync disse "10 aplicado(s)", o `/status` respondeu 37 igual ao manifest, as nove URLs deram 200 e os títulos bateram — **e o endereço canônico da página de compatibilidade continuou servindo, por horas, a cópia anterior: sem um botão de compra e com uma frase que o contrato tinha acabado de proibir em quatro cartões.** A mesma URL com uma chave de quebra de cache mostrava a página nova, inteira e correta.

A causa tinha assinatura no HTML servido — `<!--Generated by Endurance Page Cache-->`, o cache em arquivo do próprio hospedeiro, que guarda a página com `max-age` de duas horas. Ele purga quando um POST é salvo no wp-admin, e o Sync das ilhas **grava options e atualiza snippets**: nunca passa por lá. É a cicatriz de abertura desta seção uma camada abaixo — lá, "commit sem Sync não é entrega"; aqui, **Sync aplicado e `/status` batendo também não são entrega**. Naquele dia a página levou cerca de hora e meia para chegar ao leitor, e durante esse tempo todo o `/status` afirmava que estava tudo publicado.

**A regra, e ela cabe em duas linhas:**

1. **Toda ilha confere o endereço CANÔNICO contra o mesmo endereço com quebra de cache**, ao fim de todo bloco publicável. Se os dois discordarem no que o bloco mudou, o bloco **não está no ar** — diga isso no relatório em vez de dar por entregue. A régua é barata: dois `curl` por URL, um limpo e um com `?rbm_quebra_de_cache=<agora>`, comparando os marcadores que o bloco publica. Comparar bytes dá falso alarme, porque a chave aparece no canonical e nos links internos.
2. **A purga do cache do hospedeiro é responsabilidade da casca, não do Sync** — o Sync se pula a si mesmo por desenho, então correção nele não chega ao site por ele mesmo. O gatilho que funciona é a própria gravação das options da ilha. A implementação da Robometria está em `ilhas/robometria/snippets/robometria-casca.php` (`robometria_casca_purgar_cache`) e é para copiar: ela dispara os ganchos dos caches conhecidos e esvazia a pasta do cache — que é o que o `purge_all()` do próprio plugin faz —, com guarda de caminho e sem seguir link simbólico. **A atribuição é inferência, e está dita como tal no código:** a entrada presa só venceria às 16h40 por tempo e foi recriada às 16h05:59, durante o Sync em que a purga rodava. O teste que a transforma em medição está escrito lá.

**E A METADE QUE SE REPETE, porque é a mais cara:** o `conferir-no-ar.py` daquela ilha **APROVOU** a revisão 37 com 167 afirmações e zero falha, sobre uma página de horas antes. Todas as afirmações eram verdadeiras e nenhuma encostava no que o bloco tinha mudado. **Régua que não cobre o que o bloco mudou é régua que certifica a página de ontem** — então todo bloco publicável nasce com pelo menos uma afirmação, medida no HTML servido, sobre a coisa que ele mudou. Sem isso, "verificado no ar" quer dizer só que o site existe.

**E A LIÇÃO DE MÉTODO QUE SAIU DO DIAGNÓSTICO, porque ela custou quatro revisões e uma conclusão errada:** **cabeçalho e corpo de uma mesma URL se medem na MESMA requisição** (`curl -D - -o /dev/null`), nunca num `curl -I` seguido de um `curl`. Com o cache virando, as duas requisições pegam estados diferentes e a comparação **descreve dois instantes e conclui sobre um**. Foi assim que um `no-store` sem `last-modified` — que era o retrato de um arquivo de cache sendo apagado naquele segundo — passou por prova de que a camada estava fora do alcance do PHP, e uma correção que funcionava foi retirada por isso. É a mesma família do "teste que mede a si mesmo" da seção 8, um nível abaixo: aqui o defeito não é a régua, é a **janela** em que ela mede.

**E a outra metade, que é aritmética de relógio:** antes de atribuir a virada de um cache a uma correção sua, confira se ele não venceria sozinho naquela hora. `max-age` e o `last-modified` da entrada dizem quando ela expiraria; se a página trocou **antes** disso, o vencimento por tempo está descartado e sobra a sua correção — e isso é a causa mais forte que dá para ter sem acesso ao painel. Se a página trocou **depois**, você não sabe de nada, e dizer que sabe é inventar a medição.

**Também sai daqui, e vale para toda ferramenta da nuvem que lê uma ilha:** `curl` sem `Accept-Encoding` pode receber uma variante de cache diferente da que o navegador e o Googlebot recebem — na Robometria, uma cópia de **três dias antes**, sem trilha e sem `BreadcrumbList`. Medido: `gzip`, `identity` e `br` traziam a página de hoje; **nenhum cabeçalho** trazia a velha. Toda conferência no ar manda `Accept-Encoding: identity`, que traz a página de hoje sem o subprocesso precisar descomprimir nada. **Isso não fecha o defeito** — a variante quebrada continua sendo medida de propósito, senão a correção troca o defeito por uma régua que não o enxerga.
...
- **PRESTAÇÃO DE CONTAS: TODO ITEM DO BANCO É NOMEADO UMA VEZ EM CADA RESPOSTA, e a vitrine serve EXATAMENTE o que a frase nomeia (Clube do Mosaico, 12/09/2026).** A regra de elegibilidade acima decide quem entra; faltava dizer o que a página deve ao leitor sobre quem NÃO entrou — e é aí que as duas ferramentas da ilha estavam erradas, cada uma de um jeito, as duas passando por cinco portões verdes. Na F2 a frase dizia *"o rejunte é X"*, singular e definitiva, e o bloco de compra logo abaixo servia **quatro** cartões: dos 5 rejuntes do banco, 1 era recomendado, 1 era nomeado como excluído e **3 ficavam à venda sem que uma linha da página dissesse o que eles eram**. Havia ainda um quinto grupo (o de fonte fraca) que o código nunca imprimia em lugar nenhum — vazio hoje, invisível para sempre no dia em que enchesse. Na F1, a mesma conta não fechava dentro do tipo escolhido. Portanto, em toda ilha e em toda ferramenta que recomenda produto: **cada item da categoria consultada aparece exatamente uma vez na prosa da resposta** — ou na frase que o recomenda, e aí ele está na vitrine, ou numa linha que diz por que ele não está —, e **nenhum produto entra no bloco de compra sem que a frase de recomendação o nomeie**. A soma dos nomeados fecha o banco, e isso se mede: a conferência varre a entrada inteira e compara com o tamanho do banco contado, nunca com um número digitado. Vale igual para a **tabela pré-renderizada**, que é a metade que um modelo de linguagem lê sem preencher formulário — foi lá que três das nove linhas da F2 não somavam o banco, e é lá que o erro é mais caro.
- **A RECUSA NOMEIA A CAUSA QUE A PÁGINA MEDIU, nunca oferece hipótese (mesma ilha, mesmo dia).** O outro lado da moeda acima. A F1 escrevia *"Nenhum rejunte do nosso banco declara folga de N mm. Ou a folga está fora da faixa que os fabricantes publicam, ou o produto não declara essa largura"* — com as duas listas na mão, sabendo exatamente qual das duas causas valia. Nos estados em que quem excluía era o **lugar** e não a folga, a frase era simplesmente falsa, e no pior caso a página negava e afirmava o mesmo fato em duas frases seguidas: *"Nenhum … declara folga de 2 mm"*, e no mesmo parágrafo, *"De outro tipo, mas dentro dessa folga: Rejunte Epóxi"*. Duas regras saem disso: **causa que o código separa, o texto separa** (grupo por motivo, com nome próprio e produtos nomeados — misturar duas causas numa frase é inventar uma delas); e **afirmação em bloco tem o escopo do que foi medido** — ali o filtro era por tipo de produto e a frase falava do banco inteiro, o que é a mesma família do número de tela digitado. Oferecer ao leitor uma hipótese sobre um fato que a própria página calcula é confessar que a tela não leu o que o código sabe.
...
- **Rastreamento por programa.** Shopee: `Sub_id 1` = nome da ilha, `Sub_id 2` = código da ferramenta de origem. O campo do banco é o mesmo `afiliado.url`, com `afiliado.programa: shopee | mercadolivre` ao lado.
  - Mercado Livre: a **etiqueta** (o painel chama de "etiqueta"; cria-se em Afiliados → Administrar etiquetas, URL `mercadolivre.com.br/afiliados/adminlabel`, e escolhe-se no Gerador de links, URL `mercadolivre.com.br/afiliados/linkbuilder`) só aceita **letras minúsculas e números, sem hífen, sem espaço, sem maiúscula, no máximo 30 caracteres** — medido no painel em 13/09/2026, não suposto. O formato do Arquipélago é portanto `<ilha><ferramenta>` tudo junto e em minúsculas: `clubedomosaicof2`, `aquametriac3`, `robometriar1`. Uma etiqueta por par ilha × ferramenta; nunca reaproveite etiqueta entre ilhas. Onde texto antigo do contrato disser `aquametria-C3` ou `clubedomosaico-F1`, leia a forma sem hífen.
  - **ETIQUETAS JÁ CRIADAS** (13/09/2026, conta RMNATAL): `rmnatal` (a original, genérica), `clubedomosaicof1`, `clubedomosaicof2`.
  - **A GERAÇÃO DO LINK NÃO É AUTOMATIZÁVEL.** O Gerador de links do Mercado Livre carrega um reCAPTCHA e só responde a clique humano de verdade: clicar o botão "Gerar" por script não dispara requisição nenhuma (medido em 13/09 — zero requisições de rede após o clique programático, com a etiqueta e as URLs já preenchidas corretamente na tela). Portanto: preparar a lista de URLs é trabalho de máquina; **apertar "Gerar" e copiar os links é trabalho do Raphael**, e é por isso que a seção 12 põe isso na lista "Precisa do Raphael". Criar a etiqueta, essa sim, funciona por script.
...
- **RÉGUA ESCRITA PARA UM MUNDO QUE NUNCA ACONTECEU NASCE ERRADA SEM PODER FALHAR — e é o avesso da régua que morre quando o banco melhora (Robometria, 13/09/2026).** O contrato já registra que régua dependente de um caso raro do banco morre calada no dia em que aquele caso some. Este é o outro lado, e é mais difícil de ver porque a régua parece saudável desde sempre: o esquema da ilha permitia, com todas as letras, um par de compatibilidade **sem modelo no banco** — e em quatro dias de coleta o banco nunca produziu um. Duas réguas do gerador do artigo-âncora foram então escritas como se aquele caso fosse impossível, e as duas passaram por três blocos **sem nunca terem podido falhar**. No dia em que a primeira peça assim entrou, uma delas fez o gerador anunciar que quatro peças atravessavam marca — a negação exata da tese do artigo — e a outra fez duas peças de conjuntos diferentes contarem como um conjunto só, encolhendo em silêncio um número publicado como prova. **A regra: todo caso que o ESQUEMA permite e o banco ainda não tem é um caso que a régua precisa tratar hoje, não no dia em que aparecer** — e a maneira de provar que ela o trata é a mutação que **produz o mundo**, criando o caso que não existe em vez de esperar por ele. Verde sobre um mundo de um elemento só não é medição; é a ausência de contraexemplo confundida com prova. Corolário de onde o conserto mora: quando duas metades contam a mesma coisa, a afirmação que as compara fica **verde quando as duas erram juntas** — então o número publicado precisa de uma **terceira** conta, que leia a fonte por outro caminho e compare com o que foi ao ar.
...
- **RÉGUA AMARRADA A UM DEGRAU REPROVA A SUBIDA — e isso aconteceu SETE vezes na mesma ilha, no mesmo dia (Robometria, 16/09/2026).** É parente da régua que morre quando o banco melhora, e é pior de ver, porque ela não fica verde: fica **vermelha sobre uma página certa**. A escada do link de compra da seção 25.1 tem quatro degraus, e a ilha subiu dois deles em um dia — de manhã os links de busca encurtada, à tarde a ficha de produto da Open API. A cada subida, as réguas escritas quando só existia o degrau de baixo reprovaram a melhora: `teste-r1`, `teste-a1`, `teste-r2` e `teste-a2` cobravam `rbm-comprar-cru > 0`; depois de corrigidas para `rbm-comprar-busca > 0`, **as mesmas caíram de novo à tarde**, e com elas a seção 11 do `conferir-no-ar.py` e o `conferir-reservatorio-no-ar.py`, este último porque o **rótulo do botão** muda com o degrau ("Ver ofertas na Shopee" na busca, "Ver na Shopee" na ficha). Sete réguas, três famílias de arquivo, uma causa só. **Quando quatro réguas erram igual no mesmo dia, o erro não é de nenhuma delas: é de terem sido escritas quando só existia um degrau alcançável, cada uma sem saber das outras.** A regra: **afirmação sobre saída de compra conta CARTÃO contra BLOCO DE COMPRA — nunca procura a classe ou o rótulo de um degrau** — e imprime o degrau ao lado, como informação e nunca como critério. Trocar a classe por outra só adia a próxima queda: foi exatamente o que a correção da manhã fez, e a tarde cobrou. Vale para qualquer eixo em que a ilha possa **melhorar**: degrau de fonte, nível de declaração, grau de procedência.

- **E O AVESSO: TABELA DE ESTADO ESPERADO, DIGITADA, ENVELHECE CALADA (mesma ilha, mesmo dia).** `conferir-kits-no-ar.py` guarda uma tabela de "este par deve responder / este deve recusar", e uma linha dizia que Xiaomi E10 × escova lateral tinha de **recusar** — verdade quando foi escrita. Dois dias depois a coleta ALARGOU o registro `B106GL-BX`, o fabricante declara o par, a página passou a responder **certo**, e o arquivo continuou esperando a recusa de anteontem. Ninguém viu porque **ninguém rodou aquele conferidor desde a leva**. Régua de estado esperado precisa dizer de onde o estado sai: ou ela **deriva do banco**, ou a leva que muda o banco tem de rodá-la no mesmo bloco. Conferidor que não roda não é trava, é arquivo.
...
7. **No wp-admin — a lista fechada de plugins.** Instalar e ativar: **Code Snippets** (é onde vive todo o código da ilha), **Converter for Media** (WebP) e **Limit Login Attempts Reloaded**. Remover **Akismet** e **Hello Dolly**. **Site Kit by Google é OPCIONAL** desde 10/09/2026 — a medição do Search Console é pela API, na nuvem, com conta de serviço (`ferramentas/search-console.py`); o Site Kit só entra se o Raphael quiser o painel dentro do wp-admin, e a conexão OAuth é sempre um "sim" explícito dele.
   **PROIBIDOS, e o motivo em uma frase cada** — nenhuma execução instala nada fora da lista acima:
   - **Plugin de cache** (W3 Total Cache, LiteSpeed, WP Rocket): a HostGator já faz cache de página por mu-plugin, com nginx na frente; uma segunda camada faz o `/status` e o Sync servirem revisão velha, que é o defeito mais caro desta arquitetura. *(O W3 Total Cache é da Real 21, em outro servidor. Não é referência para ilha.)*
   - **Plugin de SEO** (Yoast, Rank Math, SEOPress, All in One): a casca é dona de `<title>`, meta, canonical, JSON-LD e do sitemap, com portão medindo cada um. Plugin de SEO vira um segundo dono da mesma decisão e ganha a briga em silêncio.
   - **Construtor de página** (Elementor, Divi, WPBakery): põem CSS e JS pesados na página pública e quebram o orçamento da seção 22.
   - **Jetpack, suíte de segurança (Wordfence e irmãs), plugin de schema, plugin de posts relacionados, plugin de banner de cookie**: todos põem JavaScript ou marcação própria na página pública, e a página pública é território da casca.
   **Três coisas que parecem plugin e são SNIPPET, porque o repositório tem de continuar sendo o dono:**
   - **GA4** — `gtag` impresso pela casca, com o ID de medição da ilha. É o que mede as referências de IA (`chatgpt.com`, `perplexity.ai`, `gemini.google.com`), que a seção 5 exige medir e que hoje nenhuma ilha mede.
   - **Redirecionamento 301** — `dados/redirecionamentos.json` no repositório, lido pela casca. A seção 16 exige 301 quando uma URL se move; o redirecionamento tem de morar no mesmo lugar que a árvore que o causou, não num banco que ninguém versiona.
   - **Endurecimento** — desligar XML-RPC, enumeração de usuário (`?author=N`) e o editor de arquivos (`DISALLOW_FILE_EDIT`). Três linhas, sem suíte.
...
21.8 **A 21.1 É LIDA AO CONTRÁRIO POR QUEM ESTÁ MEDINDO DIREITO, E JÁ SÃO DUAS ILHAS (14/09/2026).** Escrito depois de a Clube do Mosaico fechar um bloco inteiro concluindo, com número na mão, que *"o `dados/indexacao.md` desta ilha tem uma linha só, de 10/09; nenhum número autoriza leva nova de página nesta ilha hoje"* — e a ilha tem 11 URLs e `piso: abaixo` no cabeçalho. A Aquametria tinha feito o mesmo em 12/09, e lá o item ficou riscado com "SUSPENSO pela seção 21". Duas ilhas, três dias, o mesmo erro.

**Por que ele se repete, e por que não é desleixo:** quem lê a 14.8 — *"é essa série que autoriza dobrar a leva, manter ou parar"* — e olha uma série com uma linha só, conclui corretamente que a série não autoriza nada. **A armadilha é que "não autoriza" e "proíbe" não são a mesma frase**, e a seção 21 existe inteira para dizer qual das duas vale antes do piso. Some-se a isso que a conclusão errada *parece* a mais prudente, e prudência é o disfarce mais difícil de furar numa fábrica que tem razão em ter medo de página fina.

**A régua, e ela cabe em duas perguntas na ordem:** (1) o cabeçalho do `ESTADO.md` da ilha diz `piso: abaixo`? Então **a série de indexação não decide nada** — nem trava, nem adia, nem reduz; o que decide é o teto da 21.4 e os portões de qualidade da 21.3. (2) Só se disser `piso: atingido` é que a 14.6 e a 14.8 voltam a valer, e aí sim a série manda. **Quem for escrever "nenhum número autoriza leva nova" num documento de ilha lê o campo `piso` antes** — são dois segundos, e as duas vezes em que ninguém leu custaram um bloco cada. É a mesma família do "resumo velho lido como fato" da seção 4, com um agravante: aqui o que envelhece não é o resumo, é a régua que a pessoa lembrou.
...
> **A DÍVIDA FOI PAGA EM 23/09/2026, E NÃO PELO FEED — o feed nem existe mais (25.6 o aposentou em 16/09). Dos 39, 25 ganharam o par e 14 continuam, cada um com o motivo de hoje escrito.** O que a passada ensinou vale para toda ilha e é uma regra, não um relato:
>
> **25.4-b.1 — PAR PERDIDO SE REESCOLHE INTEIRO, NUNCA REMENDADO.** Quando um item tem `url` e não tem `url_produto`, a tentação é buscar o produto pela API e **grampear** o `url_produto` que voltou ao lado do link antigo — o campo fica preenchido, a contagem de dívida cai a zero e ninguém vê. **É fabricar a aparência de um par conferido**, e é pior que o buraco: o buraco está escrito e a aparência não. De um `s.shopee.com.br/XXXX` não se chega à ficha sem clicar, e a 25.4 proíbe clicar — então **ninguém mediu** que os dois apontam para o mesmo produto, e gravá-los lado a lado afirma exatamente isso. Ou os **dois** campos vêm da mesma oferta, na mesma chamada, ou nenhum dos dois entra. O que se perde na troca é o link antigo; o que se ganharia no remendo é um número bonito em cima de uma afirmação que ninguém sustenta.
>
> **25.4-b.2 — "JÁ TEM LINK" NÃO É MOTIVO PARA PRESERVAR; "JÁ TEM PAR" É.** A trava que impedia o conserto, na Aquametria, era a trava de atribuição da própria ferramenta de coleta: ela recusava tocar em registro que já tivesse `url`, para não apagar a história do link feito à mão. O argumento é bom — e é **falso justamente nos itens que precisam do conserto**, porque eles são `intestavel: true`: não é um link conferido que se preserva, é um link cuja saúde é desconhecida e **permanecerá** desconhecida. A pergunta certa não é se existe link, é se o par é **demonstrável**. Toda ferramenta de coleta de toda ilha pergunta pelo `url_produto`, nunca pelo `url`.
>
> **25.4-b.3 — MOTIVO ENVELHECE CALADO, E MOTIVO VELHO MANDA NÃO TENTAR.** Os 39 carregavam, desde 13/09, uma prosa dizendo que a recuperação pelo título fora *"TENTADA E RECUSADA"* porque a busca da Shopee serve casca de JavaScript. Era verdade naquele dia e deixou de ser três dias depois, quando a Open API entrou — e, enquanto esteve lá, mandava a execução seguinte **nem tentar**. É a mesma família do `bloqueio herdado do ESTADO.md lido como fato` da 20.2. Campo de motivo guarda **duas** coisas com vidas diferentes: a **causa** (por que a URL crua se perdeu — não muda nunca) e a **última tentativa** (o que a passada de hoje tentou e o que ouviu de volta — muda a cada passada). Quem escreve o campo separa as duas com um marcador e reescreve só a segunda; sem isso, ou a prosa cresce sem fim, ou a causa é apagada pela tentativa de hoje.
>
> **25.4-b.4 — "NÃO CASOU" TEM DUAS CAUSAS QUE PARECEM UMA, E SÓ O MOTIVO ESCRITO AS SEPARA.** Dos 14 que sobraram, **10 devolveram zero resultado em todos os degraus da escada** — o produto não está anunciado, e nenhuma trava foi acionada — e **4 tiveram candidatos BARRADOS** pelas travas da 25.7 e da variante. São dívidas opostas: a primeira só se resolve quando o mercado mudar, e a segunda pode se resolver com uma trava melhor ou com um registro de variante que falta no banco. Contadas juntas, viram um número que não diz o que fazer.
...
- **Open API da Shopee — LIBERADA E MEDIDA EM 16/09/2026. Deixou de ser aposta.** O `AppID` e a `Senha` apareceram no painel `affiliate.shopee.com.br/open_api` e o Raphael avisou. Endpoint: `https://open-api.affiliate.shopee.com.br/graphql`. Cabeçalho `Authorization: SHA256 Credential=<AppId>, Timestamp=<unix em segundos>, Signature=<sha256(AppId + Timestamp + Payload + Secret)>`, onde `Payload` é o corpo JSON exato enviado. É GraphQL puro: POST com `{"query": "..."}`, sem biblioteca, como em `ferramentas/ga4.py`. **A Shopee não dá suporte de uso** — a implementação é nossa.
  **ANTES DE QUALQUER COISA, O EGRESSO.** A nuvem não alcançava o endereço: o proxy de política devolvia `CONNECT tunnel failed, response 403`. O domínio `open-api.affiliate.shopee.com.br` teve de ser acrescentado à **lista de domínios permitidos da conta** (Configurações → Recursos → Execução de código e criação de arquivos → Domínios adicionais permitidos), onde já moravam `robometria.com.br` e `aquametria.com.br`. Depois disso a chamada devolve `200`. **Nenhuma rotina consegue mexer nessa lista — é passo do Raphael, e ilha nova que precisar de domínio novo esbarra nisso.**
  **ONDE A CREDENCIAL MORA — E A REGRA ANTERIOR ESTAVA ERRADA.** Até 16/09 esta seção mandava pôr `AppID` e `Senha` em **variável de ambiente da rotina**. **Não vai ali.** A própria tela que cadastra essas variáveis avisa, em letra miúda, que o conteúdo fica visível para qualquer pessoa que use aquele ambiente e que credencial não deve ser colocada lá. O lugar é um **documento privado no Google Drive do Raphael chamado `arquipelago-credenciais`**, no formato `NOME=valor`, uma por linha, com comentários e linhas em branco a ignorar. As rotinas já rodam com o conector do Drive ligado: leem o documento, extraem o que precisam e passam adiante **apenas como variável de ambiente do processo, em memória**. Nunca em arquivo, nunca em log, nunca em relatório, nunca em mensagem de commit. `git grep` antes de empurrar, procurando pelo NOME do campo, nunca imprimindo valor. **Esse mesmo documento é a casa dos tokens do Sync**, que hoje estão em texto puro no repositório público e precisam ser rotacionados.
  **O QUE ELA MUDA DE VERDADE, medido com chamada real em 16/09 às 16h07Z:** uma consulta `productOfferV2` devolve, de uma vez, `imageUrl`, `productLink` (a ficha canônica), `offerLink` (link de afiliado **já encurtado**), preço, loja e nota. Ou seja, em uma chamada: **a foto** que a seção 6 pede e nenhuma ilha tinha; **o encurtamento**, que era clique do Raphael; **a ficha de produto**, sem a qual o teste de vida da 25.4-b não existe; e **a leitura mecânica de canal brasileiro**, que é portão de régua em ilha de peça.
  **DUAS DESCOBERTAS DA MESMA MEDIÇÃO, e as duas viraram regra:** (a) **palavra-chave longa devolve zero, e zero não é erro** — `Electrolux KPCEL01 kit performance` voltou 0 resultados e `Xiaomi S10 robo aspirador` voltou 3; use escada de palavra-chave (código sozinho → marca + código → marca + modelo + tipo de peça → a chave escrita para a busca do site), pare no primeiro degrau útil e **registre em qual degrau casou**, que é procedência; item que percorre a escada inteira sem casar fica sem foto e sem ficha, com o motivo escrito. (b) **`offerLink` vem sem sub-id** — e link sem sub-id não sabe dizer de onde veio, então **não entra no banco**: ou a chamada manda o sub-id da ilha, ou fica o `url_busca` de hoje, que ao menos já tem. Link bonito sem atribuição é pior que link feio com atribuição.
  **O QUE ELA APOSENTA:** a fila `dados/links-afiliado-pendentes.md` para itens de Shopee (ver a regra do cano de links, acima) e o download do feed diário (25.3) — o feed era arquivo de 100 mil produtos que cobria mal a cauda longa; a API responde por item, na hora, com preço fresco. **O que ela NÃO aposenta:** o gerador do Mercado Livre continua com reCAPTCHA e continua sendo clique do Raphael, e o piso da 25.2 continua sendo a busca, porque é o único elo que não depende de concessão de terceiro.
  **VALE PARA O ARQUIPÉLAGO INTEIRO, MAS SÓ SE IMPLEMENTA NA ILHA EM FOCO.** A regra é de contrato e nasce valendo para todas as ilhas. A construção da ferramenta e a coleta acontecem na ilha reservada, respeitando a seção 1.2 — em 16/09/2026, só a **Robometria**. As outras herdam a regra pronta quando saírem da espera, sem repetir a descoberta.

### 25.7 A PRIMEIRA COLETA PELA API, E AS CINCO ARMADILHAS DE CASAMENTO QUE ELA ACHOU (16/09/2026)

Medido na Robometria, 73 registros publicáveis passados pela escada de palavra-chave da 25.6. **27 casaram — 12 de 1 (o código sozinho), 3 de 2, 8 de 3 e 4 de 4 — e 46 não casaram e ficaram sem foto e sem ficha, com o motivo escrito.** Isso é medição honesta, não falha: a regra é que **casamento errado no banco é pior que casamento nenhum, porque parece dado.**

A regra do despacho — *"o título tem de conter o código da peça ou o nome do modelo"* — **aprova cinco coisas erradas**, e as cinco foram pegas pela conferência com os olhos que a 25.3 manda fazer antes de gravar. Elas valem para toda ilha que colher por título de anúncio:

1. **A PEÇA QUE COBRE A PEÇA NÃO É A PEÇA.** *"Tampa Do Filtro"* casou com um filtro e *"Tampa Da Bateria"* com uma bateria: o título trazia o código do modelo e a palavra do tipo. O conserto é a **cabeça do título** — o tipo tem de ser o objeto, e não uma palavra perdida no meio. Barra também capa, protetor, carcaça, adesivo, berço e suporte.
2. **"KIT" NÃO É QUANTIDADE quando o registro não é um kit.** *"Kit Filtro Pinos e Lâmina"* casou com um filtro; quem comprasse levaria pinos e lâmina. Perde-se algum multipack legítimo, e a troca é consciente.
3. **CÓDIGO DE MODELO É CURTO E O MUNDO É GRANDE.** `S10`, `E10`, `W100` e `S40` sozinhos casaram com mangueira de ar-condicionado, máscara capilar, whey protein e fechadura de Volvo S40. Modelo exige **código + marca + o produto na cabeça do título**, e a lista de palavras que denunciam peça triplicou depois que *"Carcaça Inferior"*, *"Placa de Potência"* e *"Controle Remoto"* entraram como se fossem o ERB10, o ERB11 e o ERB20 — **loja de reposição escreve o nome do aparelho no título da peça**, e peça é justamente o produto destas ilhas.
4. **O SUFIXO FAZ OUTRO APARELHO.** `S40` casou com *"S40 PRO"*, depois com *"S40 C"*, e `S10` com *"S10+"*. São produtos diferentes — e o S40 Pro é justamente o que o portão do canal brasileiro tira da recomendação. O código não termina onde parece: `+` conta, e uma letra solta depois do código é sufixo de modelo, não palavra de frase.
5. **DOIS REGISTROS NO MESMO ANÚNCIO: no máximo um está certo, e não há como dizer qual — então os dois caem.** Casamento que não identifica um registro só não identifica nenhum.

**O SUB-ID DA SHOPEE É ALFANUMÉRICO, e ela valida.** Medido: `robometria`, `aquametria`, `clubedomosaico` e `rbmR1` são aceitos; `controle-de-bancada` e `teste_bancada` devolvem `[11001] Params Error : invalid sub id`. **Hífen e sublinhado estão fora** — e slug de ilha com hífen precisa de um sub-id próprio, sem ele.

**LARGURA E ALTURA DA FOTO SÃO MEDIDAS, NUNCA SUPOSTAS.** A seção 6 exige as duas quando há `url`, e a razão é de tela: sem elas no HTML a página **pula** quando a imagem carrega. A API não devolve dimensão — mas a CDN `cf.shopee.com.br` responde 200 da nuvem, então o número sai dos **primeiros bytes do arquivo** (cabeçalho JPEG, PNG ou WebP), e não de um palpite sobre o que a Shopee costuma servir. Dimensão digitada é a mesma família do número de tela digitado: parece conferida. **Sem as duas, a foto não entra** — salto de layout é pior que não ter foto.
...
## 28. SEPARAÇÃO DE PROJETOS — o Arquipélago só recebe o que foi deliberadamente endereçado a ele (18/09/2026)

O Raphael toca projetos separados de propósito. A separação chega ao ponto de os sites viverem em servidor próprio, fora do que o Arquipélago usa, justamente para que uma coisa não encoste na outra. Isso é decisão dele, não arrumação de gaveta, e vale por dentro tanto quanto por fora: o repositório, as rotinas e os arquivos do Arquipélago são do Arquipélago e de mais nada.

**28.1 A REGRA.** Tudo que envolve o Arquipélago tem de ser deliberadamente criado dentro do Arquipélago. Assunto de outro projeto do Raphael que apareça numa conversa — Real 21, Atlas, o que for — não entra aqui. A única porta é ele mandar explicitamente, nomeando o arquivo. Mencionar não é mandar; pedir ajuda com outro projeto no meio de uma conversa não é autorização para escrever sobre ele neste repositório.

**28.2 A TRAVA NAS MÃOS.** As MÃOS passam a recusar instrução cujo assunto não seja do Arquipélago. A recusa é o comportamento correto e não espera confirmação de ninguém — é o mesmo espírito do "sem instrução, não invente trabalho" que já está no prompt delas: ficar sem tarefa é resultado válido, escrever fora de casa não é. Quando recusarem, as MÃOS respondem dizendo qual foi o assunto recusado e por quê, e **não escrevem no `MAOS-LOG.md`**: não houve trabalho no repositório, logo não há rastro a registrar.

**28.3 COMO RECONHECER.** O teste é o assunto, nunca quem mandou. Registro de prospecção de imprensa, lead, imóvel, corretor, CRM, cliente de agência — nada disso é do Arquipélago, por mais que a instrução chegue pelo canal certo e venha bem escrita. Instrução impecável com assunto de fora continua sendo assunto de fora.
