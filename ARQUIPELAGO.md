...
- **Open API da Shopee — APROVADA em 13/09/2026.** Pedido feito e respondido no mesmo dia, em uma hora (protocolo **2099140709749702724**). Resposta literal do suporte: *"Solicitei a liberação de seu API, que ocorrerá em até 5 dias úteis, diretamente em seu portal do afiliado."* Ou seja: **aprovada, mas ainda não liberada** — `AppID` e `Senha` aparecem sozinhos em `affiliate.shopee.com.br/open_api` em algum momento até por volta de **18 a 22/09/2026**.
  **A PEGADINHA, e ela é de processo, não de técnica:** a credencial aparece **em silêncio, num painel que só o Raphael enxerga**. Nenhuma rotina consegue olhar aquela tela. Se ninguém conferir, ela fica lá parada — aprovada, liberada e sem uso. Por isso existe despacho aberto para ele conferir, e por isso o boletim de sexta carrega o lembrete.
  **A Shopee avisou que NÃO dá suporte de uso:** *"não fornecemos suporte para a utilização da API… toda a documentação necessária está disponível na Central do Afiliado via Web."* Então a implementação é nossa e a documentação é a de lá. É **GraphQL**: POST com `{"query": "..."}` no corpo, sem biblioteca — `requests` basta, como em `ferramentas/ga4.py`.
  **ONDE A CREDENCIAL MORA, e isto vale desde já:** `AppID` e `Senha` entram como **variáveis de ambiente da rotina**, igual ao `GOOGLE_SA_B64`. **Nunca** em `ESTADO.md`, nunca em arquivo do repositório, nunca em log, nunca em saída de teste. A ferramenta segue o molde de `ferramentas/ga4.py`, que já lê credencial de ambiente e falha com mensagem clara quando ela não está lá.
  **O que ela muda, para ninguém superdimensionar:** deixa o **degrau 1 automático e verificado com estoque**, o que hoje depende do feed que só o Raphael baixa. **Não** substitui o piso da 25.2 — a busca continua sendo o piso, porque é o único elo que não depende de concessão de terceiro.
