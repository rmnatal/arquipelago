...
A causa tinha assinatura no HTML servido — `<!--Generated by Endurance Page Cache-->`, o cache em arquivo do próprio hospedeiro, que guarda a página e a serve por `mod_rewrite` **antes de o PHP rodar**. Ele purga quando um POST é salvo no wp-admin, e o Sync das ilhas **grava options e atualiza snippets**: nunca passa por lá. É a cicatriz de abertura desta seção uma camada abaixo — lá, "commit sem Sync não é entrega"; aqui, **Sync aplicado e `/status` batendo também não são entrega**.
...
2. **A purga do cache do hospedeiro é responsabilidade da casca, não do Sync** — o Sync se pula a si mesmo por desenho, então correção nele não chega ao site por ele mesmo. O gatilho que funciona é a própria gravação das options da ilha. A implementação da Robometria está em `ilhas/robometria/snippets/robometria-casca.php` (`robometria_casca_purgar_cache`) e é para copiar: ela tenta os ganchos dos caches conhecidos e, porque **os ganchos não pegaram quando foram medidos**, esvazia a pasta do cache — que é o que o `purge_all()` do próprio plugin faz —, com guarda de caminho e sem seguir link simbólico.
